...
> 维护：**新坑先写 `PITFALLS.md`，这里只加一行索引**（上限约 16.5KB，已溢三次）。
...
| K | **判据挂代号 ≠ 报错不能带文案** | 只给 `kind` 时红成 ['unused-import','unused-import']，哪两处得另写脚本复现 → **代号管"红不红"，文案管"哪里红"** |
| L | **追加到"文件末尾"前先读到真末尾** | 插入点切走原函数末行 → 新用例 `NameError`。判据：追加后 `git diff HEAD` **只有 `+` 没有 `-`** |
| 28 | **静默回退成默认值** | 未知排序档位回退 = 用户读成"这软件排序坏了"而日志安静 → 一律 400；`ORDER BY` 拼串是**能跑通的注入** |
| 29 | **没验到 ≠ 验过了没问题** | 门禁按 `git ls-files` 扫 ⇒ 新文件没 `git add` 就不在名单里（**本机全绿 / CI 红**）。纪律：**跑门禁前先 `git add`**；`untracked_sources` 点名未验文件但**不算红** |

**十条心法**：① 加一个状态就有多处**口径**要同步（`_final_status`/`summarize_resources`/前端计数）。② 凡"静默"处都问：**失败会发生吗？之后有人知道吗？** 都否 → 加重试或留痕。③ **误报的代价**决定判据松紧：只下"能被证明"的结论。④ **新功能接完回头验旧前提**（fixture/断言/文档声称），别改产品迁就旧的。⑤ **卡死类第一动作是打调用栈**（`faulthandler.dump_traceback_later`）。⑥ 写测试先问：**这条路径会起后台线程吗？**（第 14/19 条）。⑦ **补丁 vs 规则**：同一个错出现第三遍就抽成可执行规则（第 18/20 条都有 AST 门禁）。⑧ **连跑红、单跑绿 ⇒ 有东西跨用例活着**，先打出"谁还活着"。⑨ **判据只能挂"代号"**：假绿＝挂"没报错"(要能报"核了几项"，`0` 算红)、假红＝挂中文子串；"没验到"与"验过了没问题"**必须是两个结果**。⑩ **判据"方向"类错要有非"照实现写"的观测面**（第 24 条）。
...
⚠️ **口径 = 收集总数**。本机 Windows **1400**（1394 passed + 6 skipped）；CI(Linux) **1401**（差额恒为 `POSIX_TERMINATION_SIGNALS` 的 SIGHUP 参数×1）。**改文档前先跑数；差分不闭合先怀疑自己。**
⚠️ **`img.xchina.io` 本机整段 403**（`text/plain` 非 CF 挑战页）→ **网络**问题，不是改版/回归。别拿它当靶子。
⚠️ 别前置 `export PATH="/usr/bin:/bin:$PATH"`（裸 `python` 会变成没 pytest 的 3.13.12）。coreutils 全无，`echo`/`git`/`python`/`date` 可用。
⚠️ **`exit 1` ≠ 有失败**：safe-delete 守卫拦"清空大目录"(阈值 50) → 被杀时**无 `FAILED`、无汇总行**。判据：进度行有没有 `F`。脚本要 `python -u`。
...
- CI `.github/workflows/ci.yml`：`backend`(`uv sync --group dev`+ffmpeg+`pytest`+`gateguard.py`+三个离线守卫) / `frontend`(`npm ci`+`build`+`test:task-query`) / `docker`。（取日志: jobs/{id}/logs 会 302 到 blob storage，重定向时要**摘掉 Authorization**，否则 401）
...
- **V39** 站点探针（五项探测 + 真站当靶子 + 三档证据门禁）；抓出第 16–19 条
...
- **V41** 对标 Immich：**排除模式 glob**(命中要计数)/**往年今日**/**重复标记**(只标记不删)/**扫描对账**(`None` ≠ `{"added":0}`)；抓出第 26/27 条 + 同族 H/I（**1309**）
...
- **V43** 回代码核"V42 声称推迟的"→ 周期巡检/sidecar/会话登录态**早就有**；真缺的两个入口：**资源库排序**（白名单 + 次级键 `r.id`，未知 400）·**保存的搜索**（`count` **0 就是 0**，读不出给 `null`+`broken`，应用时**整体替换**）；抓出第 28 条 + 同族 J（**1367**）
- **V44** 落地上一轮清单里"能纯代码完成"的六项：**文件头规范化**(dry_run 默认 + isobmff 不改)·**主色检索**(存色系代号)·**打包导出**(不截断)·**虚拟相册**(实时)·**Webhook**(不返 secret/留痕)·**自检面板**；+ 插件 SDK 文档（**1400**）
  ⚠️ 两个决策点：**图像能力一律走 ffmpeg（项目刻意无 Pillow）**；**加筛选维度时按位置转发 `library_filters` 的调用点都要同步**（不然"筛了没生效"且不报错 —— 本轮真发生过）
  ⚠️ 门禁**第四次**救场是个**新机制**：只扫 git 跟踪的文件 ⇒ 新文件没 `git add` = **没验到**（第 29 条）
- **三句话**：加站的成本在探测不在写声明；假绿＝判据挂"没报错"、假红＝挂中文子串，修法是**先变成结构**（第 24 条）；**"没有结果"与"没算出结果"必须两个值**、**规则生效要有计数**。
