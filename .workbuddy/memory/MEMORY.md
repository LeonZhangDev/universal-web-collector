...
| 8 | 认领 URL 是共享资源 | 复用 `gallery_base._match_score`；同分按采集器名 |
| 9 | 把"人看的文案"当数据 | 归类改用 `error_kind`；中文标签由后端下发 |
| 10 | `except OSError: pass` | 盖在"本来就会失败"的写入上 = 丢数据改装成静默 |
| 11 | 引用不存在的接口 | 前端引用 `/files/raw` 而它**从没实现过** → 404 被 `onerror` 藏掉 |
| 12 | 流式响应不关 = 泄漏 | 池满后所有 worker 阻塞，**任务卡死不报错** |
| 13 | 形参被局部变量遮蔽 | `_download_m3u8` 的 `info` 被预检顶掉 → HLS `resolved_url` 恒缺 |
| 14 | 测试里"顺手调真实入口" | `POST /tasks/create` 起真 worker，给下个用例写心跳 |
| 15 | 登记成"已处理"又拿"未处理"筛它 | 直播 `absorb()` 把 init 塞进 `seen` → init 永不下 |
| 16 | 从**一条**样本生成规则 | 生成层要比样本**宽**（`photos`→`photos\d*`）；形状判据取下界 |
| 17 | 序号后跟**内容哈希** ≠ 序号枚举 | `/data/<hash>/1-<sha256>.png` 改序号必 404。看残留熵 |
| 18 | 测试断言的是"**我想到的行**" | 产出类改动最后一步**目视完整输出** |
| 19 | **import 期单例**会起后台线程 | `task_manager = TaskManager()` 一 import 就活，抢 `DB_PATH` |
| 20 | **`curl_cffi` 响应不支持 `with`** | 只在最需换指纹时发作（外层有 `except` → 静默拿不到正文） |
| 21 | CI 里"真解码"用例**只跳不跑** | 装不上 ffmpeg → **CI 从不验产物内容**（V40 已修） |
| 22 | 不在任何调用链里的测试 = **不存在** | `test:task-query` 从落地起没在 CI 跑过 |
| 23 | 三个"仓库级不变量"脚本只在本地跑 | `selfcheck.py` / `add_site.py --all` / `verify_output.py` |
| 24 | **标签语义只能有一个方向** | `X` = 用例需要哪个平台 → 合法跳过是 `X != 当前平台` |
| 25 | 别拿**墙钟**当判据 | 断言里出现 `sleep` → 改成显式传时刻 |
| 26 | **"没有结果" ≠ "没算出结果"** | 无解码器 `no-decoder`/超限 `skipped`/无快照 `None`，**不许一律 `[]`** |
| 27 | **"规则生效了"要有计数当证据** | 否则"没配"和"配错了"一样 |
| A | 条件请求 ⊥ 续传 | 有 `Range` 不能带 `If-None-Match`（回 304 而非 206） |
| B | 隔离的窗口期 | `monkeypatch.undo()` 还原 `DB_PATH` → 谁那时碰库就写**用户真库** |
| C | 素材/断言的前提会失效 | 产品加了能力 → 回头问 fixture、旧断言、**文档声称** |
| D | 测试基线会漂 | 会话间"自动提交"会移动 HEAD → 别拿旧用例数当事实 |
| E | 测试替身写死签名 | 抽通用函数时旧的 `lambda *a:` 立刻炸 —— **替身不诚实** |
| F | **通过但理由已经不对** | 在**另一条**判据上报错、或更早一步就炸。比失败更危险 |
| G | 静默降级必须有痕 | `park*` 搬不动时不抛，但要让人知道 → `stats()['last_error']` |
| H | `None`(不改) ≠ `""`(清空) | PATCH 无条件写全部字段 → 提交一个字段抹掉别的 |
...
**十条心法**：① 加一个状态就有多处**口径**要同步。② 凡"静默"处都问：**失败会发生吗？之后有人知道吗？** 都否 → 重试或留痕。③ **误报的代价**决定判据松紧。④ **新功能接完回头验旧前提**（fixture/断言/文档声称）。⑤ **卡死类第一动作是打调用栈**。⑥ 写测试先问：**这条路径会起后台线程吗？** ⑦ **同一个错出现第三遍就抽成可执行规则**。⑧ **连跑红、单跑绿 ⇒ 有东西跨用例活着**。⑨ **判据只能挂"代号"**：假绿＝挂"没报错"(要能报"核了几项"，`0` 算红)、假红＝挂中文子串。⑩ **"方向"类错要有非"照实现写"的观测面**。

## 状态机 / 速度 / 去重
`pending→running→extracting→downloading→success/partial/failed`，可 paused/cancelled；采集器返回空 → failed。pause 在资源边界退出；resume 不重采不重下。看门狗判活**必须看库里的 `tasks.hb`**（`_LIVE_MANAGERS` 只是进程内列表）；`delete()` 不能提前 pop `_active`。资源级终态 `gone`（源站 404/410）**自动不重试**。
