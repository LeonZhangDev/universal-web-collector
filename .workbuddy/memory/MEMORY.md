...
本文件只做三件事：**索引 + 本机操作契约 + 一眼能认出的坑名**。命令清单与环境变量**都在 README**，这里不重复。
> 维护规则：**新坑先写进 `PITFALLS.md`，这里只加一行索引**；版本史只留"做了什么 + 抓出第几条 + 测试数"。否则会重新长到被截断（09-23 / 09-24 各溢过一次）。
...
`core/database.py`：加列**必须同时**进 SCHEMA 与 `_ADD_COLUMNS`；⚠️ **新表的时刻列 = `created_at` + REAL(epoch 秒)**，老表是 `created_time` 字符串 —— 名字带 `time` 的列会被下一个人当成老约定
...
**`core/localalbums.py` = 本地相册集唯一实现**（只读；登记/扫描/浏览/随机池/收藏）· `api/local.py`(出图只收 `(root_id, rel)`，**从不收裸绝对路径**)
...
## ⚠️ 静默坑索引（1–25 + 同族 A–G；细节见 `PITFALLS.md`）
...
## 状态机 / 速度 / 去重
`pending→running→extracting→downloading→success/partial/failed`，可 paused/cancelled；采集器返回空 → failed。cancel 留文件；pause 在资源边界退出；resume 不重采不重下。
看门狗判活**必须看库里的 `tasks.hb`**（`_LIVE_MANAGERS` 只是进程内列表）；`delete()` 不能提前 pop `_active`。资源级终态 `gone`（源站 404/410）**自动不重试**、手动可放行。规格不匹配报 400 而不是静默忽略。
...
⚠️ 想拿 pytest 汇总行：`--basetemp` 指系统临时目录下一个**不存在**的路径；**绝不指进项目目录**（986 个用例 setup 全 ERROR）。宿主包装器偶发缺失 → 长命令 `MODULE_NOT_FOUND`（**压根没跑**），加 `run_in_background=true` 绕过。
⚠️ **行尾**：`Path.write_text()` 在 Windows 上把 `\n` 翻成 `\r\n`。`.gitattributes` 是唯一来源；`downloaders/video.py` 与 `scripts/verify_output.py` 的 **HEAD 本来就是 CRLF**，别去"统一"。
⚠️ **npm 的"包目录存在"≠"包装好了"**：`@rollup/rollup-win32-x64-msvc` 可能是**空目录** → `vite build` 报 `Cannot find module`。修法见 `PITFALLS.md` V40 环境类。
...
- 先试 `git push origin main`；只在真报 `502`/`000`/`CONNECT tunnel failed` 时降级用 Git Data API（skill `git-push-via-rest-api`）。两种都**别在没核对时 `--force`**；推完与本地 `rev-parse HEAD` 对一次。
- CI `.github/workflows/ci.yml`：`backend`(`uv sync --group dev` + 装 ffmpeg + `pytest` + `gateguard.py` + `selfcheck.py` + `add_site.py --all` + `verify_output.py`) / `frontend`(`npm ci` + `build` + `test:task-query`) / `docker`。日志 `GET /repos/{o}/{r}/actions/jobs/{id}/logs`。
- ⚠️⚠️ **CI 绿 ≠ 本地绿**：差额清单见 `PITFALLS.md` V39 六（**修法是 V40 落地的**，要看 CI 实跑结果才算数）。依赖别漏：`curl-cffi`（主依赖）、`httpx2`（dev 组，`starlette.testclient` 必需）—— 漏了是**测试 collection 阶段整片红**。
...
**V28–V38** 逐版说明见 `docs/AGENT_DEVELOPMENT_GUIDE.md` 的版本史与 `PITFALLS.md` 各节（用例数走线 587→819→880→942→997→1063）。

- **V39** 新站点探针：五项探测 → **真实站点当靶子** → **三档证据门禁**；续 `--json`/`--snapshot`/`--smoke`·`drift_check.py`·`probe_feed.py`·`add_site.py`；抓出第 16/17/18/19 条（1075→1086→1146）
- **V39.5** 让"绿/红"不再可能是假的（`Gate.checked` + `Problem.kind` / `diagnose_block.step` / `drift_check` 空转非 0 退出）；抓出第 20 条（1105）
...
- **V39 的两个关键判断**：**加站的成本在探测不在写声明**（声明本体约 70 行、基类零改动）；"加站"真正的瓶颈多半是**可达性**（图集/漫画站多在 CF 后面）。
- **V39.5 / V40 一句话**：假绿＝判据挂在"没报错"（要 `checked`，`0` 算红）；假红＝判据挂在中文子串（要 `kind`/`step`/`level` 代号）。修法是**先把它变成结构**；**V40 的追加**：结构也有方向 —— 见第 24 条。
- **下一步候选**：更多站点插件（按需求，先跑 `probe_site.py`）/ HLS 直播（`EVENT` 或无 `ENDLIST`，缺真站样本）/ 直播断点续录（低）/ 本地相册的 dHash 去重与 EXIF 视图（**"暂时不做"，不是"不该做"**）。
