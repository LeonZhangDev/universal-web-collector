...
## V42（2026-09-25）：五个方向各对标一次，吸收五项

### ⚠️ 先分清"谁跟我们是同一件事"，再谈吸收

两轮对标里最值钱的一条经验。**Immich 的 External Library 与我们本地相册集是同一个契约**
（`:ro` 挂进来、只建索引、一个字节都不动），所以它的每个设计都能直接搬；而 Google Photos
是"你把所有照片交给它"的**托管模型**，它的"回忆"建立在全量人脸与地理之上 —— 照抄那个形态，
等于先要求用户把目录交出来, 而那正是我们只读承诺要避免的。

同理 Czkawka 的 "bad extensions" 能直接搬, 是因为它和我们一样面对**不可信来源的文件**;
而 dupeGuru 面向"我自己攒出来的重复", 它的**默认动作是删** —— 那个默认在我们这儿不能用
(dHash 会误判, 纯色图互相距离 0)。**同类产品的默认动作不能照抄。**

### 第 28 条 ⚠️ "文件叫错了名字"是独立于"内容坏了"的一类

`core/filekind.py`（扩展名声称的格式 vs 文件头实际格式）。现场: CDN 回了 **HTML 错误页**,
而 `Content-Length` 正好是那个 HTML 的长度 —— 下载层的长度校验**通过**, 文件被当成图片
落盘, 名字 `00001.jpg`。之后缩略图 / dHash / manifest 全建立在这份 HTML 上, **全程不报错**;
只有用户双击打开时才看到"文件已损坏"。

所以单列 `KIND_MISMATCH`, 与 `corrupt` **分开计数**: 缺失/截断重下有救, 这一类重下还是它。
合成一个数字就没法指导下一步动作。

判据的松紧与 `mediacheck` 同款(**宁可漏报, 不可误报**): 不认识的扩展名、认不出的文件头
一律不下结论; `ftyp` 家族(mp4/mov/m4a/heic/avif)视为同一族 —— 靠 brand 细分会把 `.heic`
判成"其实是 mp4", 那是误报: 它本来就是那个文件, 只是我们没细看。

### 体检视图的判据: 0 就是 0

`SPECIAL_FILTERS` 把九条跨字段判断各定义一次。`GET /library/facets` 每条都带 `n`,
而且 **空库上九维都在、全是 0** —— 返回空列表会被读成"没法数"(第 26 条)。
评分区间同理, **六档全给**(0..5), 否则"5 星有多少"这个问题没法问。

未知维度键 **抛 ValueError → 400**, 不是静默忽略: 静默忽略的表现是"点了没反应",
那比报错难查得多。

### 重复分组: 只说"它们重复"是把最难的一步丢回给人

`duplicate_of` 原本是一条**平铺的指向**。`duplicate_groups()` 把它还原成组并给建议保留的
那一条: 像素最高 → 体积最大 → id 最小(最早, 最像原件)。dupeGuru 的做法是每组固定一个
不可删的 reference, Czkawka 用 `-D AEN/AEB` 让用户选保留策略 —— 都在说**"留哪个"才是用户
真正要做的决定**。

⚠️ `keep_reason` 是**代号**, 中文由 `reasons` 下发。三条理由各有一条用例走它自己的那条路 ——
混在一个断言里, "理由对不对"其实没人验。

### URL 归档: 它**不是**资源库的下游副本

归档里的 URL 在本机可能根本没有对应文件(那是别人/别的机器下的)。所以它只用于
**提示与可选的跳过**, 永远不参与"这个文件在不在、能不能删"的判断 —— 那归
`resources.local_path` 管。导入回 `added` + `skipped` 两个数(第 27 条)。

### ⚠️ 顺带: `Dict` 没 import, 只有 import 期才炸

`schemas.py` 里 `DuplicateGroupOut.members: List[Dict[str, Any]]` 用的 `Dict` 当时没在
typing import 里。`py_compile` 看不出来(NameError 是运行期), 只有真 import 才炸 ——
**新增 pydantic 字段后要真的 import 一遍**, 不能只看编译。

### ⚠️ 文档上的"推迟理由"会过期(C 类的文档面)

V41 把"评分"归到"需要'属性的属性'语义"那一档而推迟; V42 证明它只是 `resources` 上的一列
整数(收藏=布尔分两堆, 评分=序数能排序)。V42 转手又把"智能文件夹"推到同一档, V43 又证明
**平铺版**(存一组筛选条件)不需要那套语义 —— 它只是把九个字段序列化下来; 真正需要的是
**规则嵌套/组合**。**同一个错法犯了两次**: 把"一件更大的事"的理由安在"其实很小的一步"上。
已在指南 §23.8 / §24.4 加更正块。规则: **推迟理由要写在"最小可做版本"旁边** ——
否则它保护的是想象力, 不是判断。

## V43（2026-09-25）：资源库的排序与保存的搜索；抓出第 28 条 + 同族 J

这一轮的起点是"**回代码里核一遍 V42 声称推迟的东西**"。核出来三条**其实早就在**：

| 以为没做 | 实际位置 |
| --- | --- |
| 周期巡检 / 定时重采 | `watches` 表 + `db.due_watches()` + 增量模式 + `interval_minutes`，挂在 `task_manager._scheduler_loop` |
| 资源级 sidecar(yt-dlp 的 `.info.json`) | `core/manifest.py` 的 `album.json`(集合级) —— 与 `manifest.json`(文件级)分工 |
| 会话 / cookie 登录态 | `core/sessions.py`：真实浏览器登录一次，存 Playwright `storage_state` |

真缺的是两个**入口**(Eagle / digiKam / PhotoPrism / Immich 都有)，不是能力。

### 第 28 条：静默回退成默认值

`GET /library?sort=` 的取值来自前端。**未知代号回退到默认档位**看起来最"友好"，
实际是最坏的一种: 用户点了"按大小排"却拿到加入顺序，他读到的结论是
**"这软件的大小排序是坏的"**，而日志里一个字都没有 —— 这正是"不报错、只是结果错"
的定义。所以未知代号一律 **400**，与 `special` 未知键同一条规矩。

⚠️ 更危险的是同源问题: `ORDER BY {前端传来的字符串}` 是**能跑通的注入** ——
结果对、不报错，所以没人会发现。取值只能来自 `LIBRARY_SORTS` 这一张白名单表，
`library_order_by()` 是**唯一翻译点**(第 7 条在排序上的实例)。

### 同族 J：不该数的时候不要数

"0 就是 0"(第 26/27 条)不能反过来用。保存的搜索列表要给 `count`，但**条件在库里
读不出来时绝不能去数**：空条件数出来的是**整个库的条数**，于是这条坏搜索会被伪装成
"一条正常但很宽的搜索" —— 用户看到四位数，只会以为自己的条件存丢了。

所以是 `count=null` + `broken=true`(没算出结果)，并且坏的那条**不隐藏**
—— 让它消失等于把"坏了"变成"没有"。
...
1. **除主键档位外，每档都要带次级键 `r.id`**。同值行(同一秒落盘 / 同样大小 / 同为 5 星)
   在 SQLite 里的相对次序由扫描顺序决定，翻页时同一行可能**出现在两页**、也可能
   **一页都不出现**。这类错不报错，只表现为"我明明看到过那一条"。
   用例用 `limit=1` 逐页取，再和整表逐位比 —— 只有这样才能验到。
2. `/library` 要**回显实际生效**的 `sort`/`order`。不回显的话，一次被兜底的请求会让界面
   高亮一个错的档位；而"高亮"与"真实顺序"不一致时，人是不会去怀疑界面的。

### 保存的搜索：条件是**具名参数**，不是 SQL

`library_searches(id, name, params(JSON), created_at)`。时刻列按新表约定 =
**`created_at` + REAL(epoch 秒)**。

* 存 JSON **不等于**存了一段可执行的 SQL: 回灌时仍是 `library_count(**params)` /
  `library_filters(**params)`。白名单(`LIBRARY_QUERY_KEYS`)之外一个键都进不来。
* **坏条件必须在保存时拒绝**: 一个存进去的非法 `special` 会让*每次列出搜索*都抛异常
  —— 整个侧栏打不开。保存时拒绝是唯一能保证"存下来的都跑得动"的位置。
* 同名**覆盖**而不是报"已存在"(用户的常态是"改完条件再存")，但要如实回
  `created=false`，让他知道刚才那条被换掉了。
* 应用一个搜索 = **整体替换**所有筛选条件，**不是合并**。侧栏上的 `count` 是按
  "存下来的那组条件"算的; 带着上一轮的残留条件去查，列表条数与那个数字对不上 ——
  而这**不报错**，只表现为"这个搜索的数好像不太准"。
* 手动改任何筛选条件都要**取消当前搜索的高亮**，否则侧栏一直亮着"搜索 A"而列表
  早就是别的东西了。
...
## V45（2026-09-26）：排除筛选 / 日期区间 / 找相似 / 投递历史 / cron / 幻灯片

起点还是那套: 先**回代码确认这产品有几个方向**，再去搜同类产品，最后只吸收
跟我们同一契约的那部分。这一轮扫出来的**五个假阳性**值得单独记 —— 它们都是
“关键词命中了、打开一看早就有”:

| 命中 | 实际 |
| --- | --- |
| `core/imageinfo.py` | 名字像 EXIF，实际**只解宽高** ⇒ “拍摄时间”这一列根本不存在，日期筛选的中文只能写“落盘” |
| `trash` | 排除表(`SKIP_DIRS`)，不是回收站 |
| `proxy` | 网络代理(`proxy_health.py`) |
| `map` / `geo` | Python 内置的 `map()` |
| `face` / `semantic` | 只出现在 `.md`(对标笔记)里 |

还差点提错一条: 想写“补视频封面帧”，一核 `core/thumbs.py` 早就有了。
**推迟理由会过期**(V41 评分 / V42 智能文件夹都是被“回代码看要多少活”戳破的) ——
所以“暂时不做”旁边必须写“最小可做版本”。

### 第 30 条：选择器前缀对不上 = 整段样式静默失效

`style.css` 里灯箱那一组写的是 `.lightbox img` / `.lightbox .nav` /
`.lightbox .lb-close` / `.lightbox .lb-count`，而组件根元素的类名是
**`lightbox-mask`** —— 整套后代选择器**一条都没生效**(导航箭头、关闭叉、计数、
图片尺寸约束全是浏览器默认值)，`.lb-tools` / `.lb-btn` / `.lb-zoom` 则根本没有样式。

特征与本项目反复踩的那些完全一样: **页面照常出来、控制台一个字都没有**，只会
让人觉得“这个灯箱比较丑”。它是加幻灯片控件时才被看见的 —— 新按钮没样式才去查。

> 判据: 选择器前缀必须是**组件根类名**。CSS 没有“样式没生效”这种报错，所以
> 这类错只能靠“新加的控件长得不对”偶然撞见 —— 加 UI 时顺手核一眼前缀。

### 排除语义：`NOT EXISTS`，不是 `EXISTS(NOT …)`
...
还有半边: 排除时 `NULL` 必须算“不是”。`NULL <> 'x'` 在 SQL 里既不是真也不是假，
所以每条排除都写成 `(r.type IS NULL OR r.type <> ?)`。漏掉这半边的表现是
“排除了视频却少了一批图”(那些 `type` 为空的记录被连带排掉了)。

### 相似与重复是**两个阈值**

`phash.DEFAULT_THRESHOLD = 4`(就是同一张) 与 `SIMILAR_MAX_DISTANCE = 12`(看着像)
必须分开: 相似里混着“构图像但不是同一张”的图，当成重复处理会**真丢东西**。
另外 dHash 只看**梯度**，纯色图指纹恒为 0、互相距离 0 —— 退化指纹要挡在比对之外
(同族 I 的第二次出现，这次在“相似检索”这条新路径上)。

`ok=false` 一定回 `reason` + `reason_label`: `no-fingerprint` / `degenerate` /
`no-decoder` / `not-found` 都是“**没法比**”，不是“没有相似的”(第 26 条)。
候选被截断要报 `truncated` + `scanned`(同族 J: 不该数的时候不要数，但**截断了要报**)。

### `applied` 的形状改过一次

第一版 `library_filters` 返回一串**裸键名**，前端照着渲染就会露出英文键。改成
`[{"key":…, "label":…}]`，中文只在 `LIBRARY_FILTER_LABELS` 一处翻译(第 9 条)，
并加了一条断言钉住: `applied` 里出现的每个键都必须在标签表里有中文名 ——
**键名对不上不报错、测试也照样绿**，只在界面上露一枚英文胶囊。

> 通用形态: 后端回灌给界面的结构里**不许出现需要界面二次翻译的代号**。

### cron 三个最容易错的点(零依赖自研 `core/cron.py`)

1. 日与周**同时限定取“或”**(标准 cron 语义)。取“且”的话 `0 9 1 * 1` 要等
   “既是 1 号又是周一”，一年只跑一次。
2. cron 的周日是 `0`，Python `weekday()` 的周日是 `6` —— 必须平移，否则周日=0
   永远匹配不上。
3. 周日写 `7` 归一到 `0`，且必须**在展开之后**做(先归一会把 `7` 当越界值丢掉)。

另外 `next_after` 收字符串会炸(`.replace(second=…)` 要 datetime)，加了
`_as_datetime()` 统一转换 —— 与 `_minutes_later` 是同一个历史坑: **两种入参
形态必须在一处归一**，否则只在“恰好传了另一种”时才炸。

校验只在后端: 它回 400 并带**中文原因**(同族 K: 代号管红不红，文案管哪里红)。
前端另写一套解析器的下场是“前端说合法、后端说不”，表现为订阅静默不跑。

### `sqlite3.Row` 又踩了一次

`similar_to()` 里写了 `row.get("local_path")`，运行时 `AttributeError`。这个坑
仓库里早就记过，但它只在**有真指纹才走到**的路径上发作 —— 平时跑不到。
统一写法: `row["local_path"] if "local_path" in row.keys() else None`。
