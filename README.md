...
| GET | /library | **跨任务资源库**: `q`(搜索) / `kind`(image,text) / `album`(相册名精确匹配) / `task_id` / `tag` / `tag_children`(连子标签) / `favorite` / `min_rating` / `special` / `sort`+`order`(排序, 档位见下) / `page` / `page_size`; 默认只列 `done` 资源, 每条带 `refs`(被多少任务共用) 与 `tags`; 回显**实际生效**的 `sort`/`order` 与 `sorts` 档位表 |
| GET | /library/searches | **保存的搜索(智能文件夹)**: 每项带 `count`(**0 就是 0**; 条件读不出来时给 `null` 并置 `broken`, 与"匹配 0 条"是两个结论) |
| POST | /library/searches | **新建/覆盖保存的搜索** `{"name":"...","params":{...}}`; 同名覆盖。条件在**保存时**就过白名单与合法性校验(否则一个坏搜索会让每次列表都抛异常) |
| DELETE | /library/searches/{id} | 删除保存的搜索(幂等: `deleted=false` 表示本来就不存在) |
| GET | /library/albums | 资源库内出现过的相册名(仅含有已完成资源的相册) |
| POST | /library/rate | **批量打星** `{"ids":[...],"rating":0-5}`; `0` = 清除评分。上限由后端硬拦(越界 400), 界面只画星星 |
| GET | /library/facets | **体检视图**: 未打标签 / 未评分 / 疑似重复 / 内容损坏 / 名字与内容不符…每个维度带**真数过的** `n`; 另给 0-5 星各档条数。`items[].key` 可直接喂 `GET /library?special=` |
| GET | /library/duplicates | 疑似重复**分组**视图: 每组给建议保留的一条 + `keep_reason` 代号(像素最高/体积最大/最早), 中文由 `reasons` 下发。只标记不删 |
| GET·POST·DELETE | /library/url-archive | **已下载 URL 归档**(对标 `--download-archive`): 导入回 `added`/`skipped` 两个计数, 导出纯文本可带到别台机器; 只用于"我记得下过", 不参与"文件在不在"的判断 |
...
| POST | /library/verify | **落盘后完整性巡检**: 用 `mediacheck` 判缺失/截断, 再用 `filekind` 判**名字与内容不符**(扩展名说是图片、文件头是 HTML); **只标记不删**, 三类分开计数; 可按 `task_id` 限定范围 |
...
V42 起它再判第三类: **名字与内容不符**(`core/filekind.py`, 扩展名声称的格式与文件头
对不上)。最典型的是 CDN 回了 HTML 错误页而 `Content-Length` 正好是那个页面的长度 ——
长度校验通过, 文件被当成图片落盘, 之后缩略图与指纹全建立在这份 HTML 上, 全程不报错。
三类**分开计数**: 前两类重下有救, 第三类重下还是它。

> 为什么"只标记": 巡检的判据是算术推断, 误报的代价是删掉一个好文件。V33 建立的原则
> 在这里继续适用 —— 只下"能被证明"的结论, 认不出的一律放行。`filekind` 同款取舍:
> 不认识的扩展名、认不出的文件头, 一律不下结论(`ftyp` 家族视为同一族, 细分只会误报)。
...
`resources` 再加一列 `rating`(0-5, **`0` = 未评分**)。它与收藏**并存而不是合并**:
收藏只能把东西分成"要/不要"两堆, 而在几千张里挑"最好的那几张"时需要的是**能排序**
的序数 —— 布尔做不到排序。上限由**后端硬拦**(越界 400), 界面只画星星。

`SPECIAL_FILTERS` 把九条**跨字段判断**各定义一次(未打标签 / 未评分 / 已评分 /
疑似重复 / 内容损坏 / 文件已丢失 / 名字与内容不符 / 没量到尺寸 / 没量到时长),
`GET /library/facets` 给每一条的条数, `GET /library?special=<key>` 消费它:

* 这类问题**不是某个字段的取值**, 在字段级筛选里根本没有入口 —— 指望用户自己拼出
  "没有标签 AND 类型是图片"是不现实的
* 每个维度的 `n` 是**真数过的**, **0 就是 0**; "没有这类"与"没算出这类"是两个结论
* 未知键**报 400** 而不是静默忽略 —— 静默忽略的表现是"点了没反应"
* 中文标签与 `keep_reason` 的中文都由后端下发, 前端只拿键查表

### 资源库排序 / 保存的搜索(V43)

`GET /library?sort=&order=`, 档位定义在**唯一一张表** `LIBRARY_SORTS`(加入顺序 /
文件大小 / 星级 / 文件名 / 类型), 中文名与默认档位随 `/library` 与 `/library/facets`
**同源**下发:

* 前端传的是**代号**, 后端在白名单里翻译成 `ORDER BY` 片段 —— 拿前端传来的字符串
  直接拼 SQL 是能跑通的注入, 所以没人会发现; 未知代号**回 400**, 不静默按默认排序
  (静默回退会被读成"这软件的大小排序是坏的", 而日志里一个字都没有)
* 除主键档位外**每一档都带次级键 `r.id`**: 同值行的相对次序由 SQLite 扫描顺序决定,
  翻页时同一行可能出现在两页、也可能一页都不出现。这类错不报错, 只表现为
  "我明明看到过那一条"
* `order` 只认 `asc`, 其余一律当 `desc` —— 方向写错用户一眼能看出来, 而为此回 400
  只会让前端多一条要处理的错误路径

**保存的搜索(智能文件夹)**把一组筛选条件命名存下来(`library_searches` 表), 侧栏一键
复用。存的是**具名参数**(JSON), 回灌时仍走 `library_filters` 的具名参数, 所以永远不会
变成拼出来的 SQL:

* 列表里每条都带 `count`: "配了但一条都不匹配"与"没配"在界面上必须能分开, **0 就是 0**
* 条件读不出来(手改过库/旧值)时给 `count=null` + `broken=true`, **不是 0** ——
  空条件数出来的是"整个库的条数", 那会把一条坏搜索伪装成"正常但很宽", 用户看到四位数
  只会以为自己的条件存丢了
* 条件在**保存时**就过白名单与合法性校验: 一个存进去的非法 `special` 会让**每次列出
  搜索**都抛异常, 整个侧栏打不开
* 应用一个搜索 = **整体替换**所有筛选条件, 不是合并。带着上一轮的残留条件去查的话,
  列表条数会与那个 `count` 对不上, 而这不报错, 只表现为"这个搜索的数好像不准"
* 手动改任何筛选条件会**取消高亮**当前搜索 —— 否则侧栏一直亮着"搜索 A"而列表早就是
  别的东西了

### 资源库: 排除筛选 / 日期区间 / 找相似 / 投递历史 / cron(V45)

**排除筛选** `GET /library?exclude_tag=&exclude_album=&exclude_kind=&exclude_special=&exclude_color=`。
此前条件之间只有"并且", 想说"除了这个标签之外都要"只能先列全库再手工跳过。

* 排除的语义是「**没有**这个标签」, 用 `NOT EXISTS` 实现 —— **不是** `EXISTS(NOT …)`。
  后者会把"有 A 也有 B"的资源留下来(任意一条不等于 A 就成立), 等于没筛。
* 排除时 `NULL` 必须算"不是": `NULL <> 'x'` 在 SQL 里既不是真也不是假, 所以每条都写成
  `(r.type IS NULL OR r.type <> ?)`。漏掉这半边的表现是"排除了视频却少了一批图"
  (那些 `type` 为空的记录被连带排掉了)。
* 未知键同样**报 400**。排除的"没生效"与"本来就没这类"长得一模一样, 静默忽略没法区分。

**落盘日期区间** `date_from` / `date_to`(`YYYY-MM-DD`, 含两端):

* 口径是**落盘时间**, 不是拍摄时间 —— 我们没有 EXIF(`core/imageinfo.py` 只解宽高),
  所以排序档位与筛选标签的中文都写"**落盘**"。写"拍摄"是安静地错。
* 上界补到当天 `23:59:59`: `created_time <= '2026-09-26'` 会把当天 00:00 之后落盘的
  全部排除, 表现为"我选了今天却一条都没有"。
* 形态强制 `YYYY-MM-DD`, 别的一律 400 —— 宽松接受输入的话, "2026-9-1" 今天能过、
  明天换个格式就查不到东西。

**生效条件回显** `GET /library` 多返回一个 `applied`(第 27 条: 规则生效要有证据)。
它由**生成 SQL 的那一处**(`library_filters`)给出, 界面拿它渲染胶囊, 不按参数真假自己猜:
`kind="all"` / `color="all"` 都是"不限", 照参数画会出现一枚"类型: 全部"的胶囊, 用户以为
自己筛过了。键 → 中文的映射只有 `LIBRARY_FILTER_LABELS` 一份。

**找相似** `GET /library/{id}/similar`: 按 dHash 汉明距离返回"看着像"的图。

* ⚠️ 与"疑似重复"是**两个阈值**: 重复 `≤4`("就是同一张"), 相似默认 `≤12`("看着像")。
  相似里混着"构图像但不是同一张"的图, 当成重复处理会真丢东西。
* `ok=false` 时一定带 `reason`(`no-fingerprint` / `degenerate` / `no-decoder` / `not-found`)
  与 `reason_label`: 那些是"**没法比**", 不是"没有相似的" —— 一律显示成空列表会把
  两者混成一个答案(第 26 条)。
* 候选超过扫描上限时回报 `truncated` + `scanned`, 不许静默少比(同族 J)。
* 纯色/低梯度图的指纹是退化的, 直接挡在比对之外 —— dHash 只看梯度, 两张纯色图距离恒为 0。

**Webhook 重试与逐次投递历史**:

* 4xx **不重试**(再发一次还是 404), 5xx / 429 / 网络失败重试, 退避 `(0, 0.6, 1.8)`。
* 每次尝试**各留一行**(`webhook_deliveries`), 不合并 —— 合并之后"第一次超时、第二次成了"
  就看不见了, 而那句正是在说对端有问题。`GET /webhooks/{id}/deliveries` 取。
* 删 webhook 时**显式先删历史**: SQLite 的外键默认是关的, `ON DELETE CASCADE` 不生效,
  不先删就会留下一批指向不存在的 hook 的孤儿行。

**订阅支持 cron** `POST /watches` 多收一个 `cron`(5 段): 给了就**按 cron 排下一次**,
`interval_minutes` 只作兜底。零依赖自己解析, 三处最容易错的地方:

* 日与周**同时限定取"或"**(标准 cron 语义)。取"且"的话 `0 9 1 * 1` 要等"既是 1 号又是周一",
  一年只跑一次。
* cron 的周日是 `0`, Python `weekday()` 的周日是 `6` —— 必须平移, 否则周日=0 永远匹配不上。
* 周日写 `7` 要归一到 `0`, 且必须**在展开之后**做(先归一会把 `7` 当成越界值丢掉)。

不合法的表达式回 400 并带**中文原因**(第 K 条: 代号管"红不红", 文案管"哪里红")。
前端不另写一套解析器 —— 两套规则迟早不一致, 而"前端说合法、后端说不"的表现是订阅静默不跑。

**幻灯片自动播放**: 灯箱工具条上多了播放键与间隔选择(3/5/10/20 秒)。默认**不播**(自动翻页
是"程序在替你翻"), 只有 1 张时按钮禁用(`go()` 在 `length<=1` 时直接 return, 播了也看不出
动静), `onUnmounted` 清定时器 —— 不清的话组件销毁后仍会 `emit("update:index")`, 变成
"关掉灯箱后列表自己在动"。

### 顺带修掉: 灯箱整套 CSS 选择器前缀对不上(真缺陷)

`style.css` 里灯箱那一组选择器写的是 `.lightbox img` / `.lightbox .nav` / `.lightbox .lb-close`
/ `.lightbox .lb-count`, 而组件根元素的类名是 **`lightbox-mask`** —— 整套后代选择器
**一条都没生效**(导航箭头、关闭叉、计数、图片尺寸约束全是浏览器默认值), 工具条
`.lb-tools` / `.lb-btn` / `.lb-zoom` 则**根本没有样式**。

这类错的特征: 页面照常出来、控制台一个字都没有, 只会让人觉得"这个灯箱比较丑"。
修法是把前缀改成 `.lightbox-mask` 并补齐缺失的工具条样式。
