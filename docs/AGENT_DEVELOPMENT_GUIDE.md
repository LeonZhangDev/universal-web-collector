...
| 全量 pytest | 本机 Windows 收集 **1341** → **1335 passed + 6 skipped**（V42 新增 `tests/test_features_v42.py` 32 项）；CI/Linux 收集 **1342**(差额仍是 `POSIX_TERMINATION_SIGNALS` 的 SIGHUP 参数) |
...
* **语义搜索 / 人脸聚类 / 地图**: 另一档投入(模型 + 索引 + 一套新 UI), 且地图涉及
  合规。
* **颜色搜索 / 智能文件夹**: 需要一套"属性的属性"语义(规则的组合与嵌套), 那是
  比一列 `rating` 大得多的设计。**注意: 上一版把评分也归到这一档是记错了** ——
  评分只是整数列, 已更正(见 §22.8 末尾的更正块)。
