...
> 🔁 **更正（V34 起失效）**：第 2 条**已经不成立**。熔断状态现在落在
> `core/proxy_health.py`（`_meta` 下的 JSON，走 `core/jsonstore.py` 的四条并发纪律），
> `ProxyPool.__init__` 调 `_load_state()` 载入，`note_failure` / `note_success` 落盘。
> 触发这次改动的是一个真实取舍：**每轮重启都把"这条线不行"忘掉，代价是几个真实资源白下**；
> 而"写 SQLite 撞单写者"这个理由本身就站不住 —— 它需要一个**文件级**的答案（`jsonstore`），
> 而不是"干脆不持久化"。
>
> 保留第 1 条不变（全池熔断时继续直连，而不是返回 `None`）—— 那条经得起复核。
>
> 这条更正与 §14.5 是同一型：**"只放内存""暂不支持""还没有"都是有保质期的话**，
> 写在历史章节里不带更正注，就会被后来的人当成现状。
...
### 14.5 ⚠️ 文档复核：四处"声称缺失/已经变了"的地方

复核 backlog 时逐条去代码里核，而不是读着文档改文档。第一轮修掉两处，第二轮（提交前
再做一次全文扫描）又扫出两处：

| # | 位置 | 文档声称 | 实际 |
| --- | --- | --- | --- |
| 1 | `PROJECT_OVERVIEW` 后续可做 | 站点级并发配额"没有'这个站最多几个在跑'的显式约束" | `DomainLimiter` 按 `site_key` 持有 `BoundedSemaphore(settings.domain_concurrency)`，默认 3，`slot()` 确实 `acquire` |
| 2 | `PROJECT_OVERVIEW` 后续可做 | 巡检结果"即时返回，刷新即丢" | `/library/verify` 直接把 `error_kind` + `note` 写回 `resources` |
| 3 | `README` 代理熔断 | "熔断状态只放内存不落库: 进程重启即重算" | V34 已落盘 `core/proxy_health.py` |
| 4 | **本文档 §11.4** | "只放内存不落库"（**同一份文件**的 §13.7 却写着 V34 已统一到 `jsonstore`） | 同上 —— 典型的**自相矛盾** |

第 4 条最值得记：它不在 "后续可做" 那种"清单型"位置，而是埋在**历史章节的设计说明**里。
清单型位置容易被想起来复核，"当初为什么这么设计"的段落不会 —— 而后者读者更容易信。

顺带修掉一个**自己造成的结构缺陷**：V34 那轮插入 §13 时标题写了 `# 13.`（旧式 H1），
子节写成 `## 13.x`，与相邻的 §12/§14 的编号方案（`##` + `###`）不一致，导致 §13 在目录
里和全文同级、子节和章节同级。**改文档同样要遵守文档自己的约定** —— 这类错误不会让
内容变错，但会让结构不可信。

> **这与"fixture 不诚实""旧断言前提失效"是同一型：声称某能力不在，其实早有。**
> 唯一可靠的防法是**逐条去代码里核**，并定期做一次全文关键词扫描
> （`暂不支持 / 还没有 / 只放内存 / 未实现 / 缺一个`）—— 本次两轮共扫出 4 处，
> 每一处都对应一个已经变了的事实。
...
## 15. V36 — 分片缓存复用 / 标签层级与颜色 / DASH 字节区间与多时段

V35 收尾时列了三条 backlog（见 PROJECT_OVERVIEW「后续可做」）。这一轮把它们做完，
并顺手修掉两个东西：一个是 HLS 路径**丢掉调用方 `info`** 的静默缺陷，一个是
「暂存区永远攒不起来」没有任何线索的观测盲区。

三条建议有一个共同的形状：**它们都是"把已经存在的机制用到第二个地方"**。
所以这一节的重点不是功能，而是*复用时不许重新发明*。

### 15.1 分片缓存的跨任务复用：复用暂存区，不写第二套

原来的形状：HLS/DASH 的分片缓存躺在**目标路径旁边**（`.<stem>.parts/`）。
取消之后它还在，但**只在"下次还是同一个相册名、同一个文件名"时才找得回来**。
用户改一次命名模板，几百 MB 字节就白下了。

`core/partials.py` 在 V35 已经有一套完整的"按 URL 寻址 + TTL + 总量预算 + 最久没用先走"
的纪律。**这一轮要做的是把分片缓存接进去，而不是给它写第二份 TTL/预算逻辑** ——
两份迟早漂移，而漂移的表现是"某个类型永远不淘汰"。

落点：

```
_meta/partial/{sha1(清单URL)}.part     # 单文件（V35 已有）
_meta/partial/{sha1(清单URL)}.seg/     # 分片缓存（本版新增）
```

索引条目加一个 `kind` 字段（`file` / `segments`），`stats()` / `sweep()` / `clear()`
全部按它分派落点。`_entry_path(kind, entry)` 是唯一知道"两类内容各自长什么样"的地方。

**地址的取值维度**：单文件用资源 URL；分片缓存用**清单 URL**（m3u8 / mpd），DASH 还要
加轨后缀（`#video` / `#audio`），多时段再加时段后缀（`#p1`）。理由与 §14 的
`ensure_fingerprint` 同源：**两条轨各有各的一份，不能互相顶掉** —— 顶掉之后拼出来的
是"视频头 + 音频身"。

三个必须守住的点：

* **清单指纹**。同一个清单 URL 下的分片换代是最隐蔽的错配。`fingerprint()` 把分片清单
  哈希一下存进 `manifest.fp` 标记文件；不符**整份丢弃**。不做"按序号挑能对上的"那种
  聪明事 —— 那正是把两批字节混成一份的入口。指纹要支持**字节区间**分片
  （`(URL, (起, 止))`），所以序列化时把区间也喂进哈希，并且**用换行分隔**：
  直接相连会让 `["ab","c"]` 与 `["a","bc"]` 同哈希。
* **搬不动不报错，但要留痕**。`park*` 失败是刻意的静默降级（原地那份不动，最坏只是
  下次从头下）。可那样一来"暂存区怎么永远攒不起来"就没有任何线索 —— 于是失败原因记进
  模块级 `_LAST_PARK_ERROR`，由 `stats()["last_error"]` 带出来。
  （这是本项目那条"凡静默处问两个问题"的直接应用：失败会发生吗？会。失败之后有人知道吗？
  原本**否**，所以补上留痕。）
* **取消信号要显式穿透**。`except BaseException` 收字节之前必须有一条
  `except TaskCancelled: raise`（第 4 条静默坑，`tests/test_cancel_guard.py` 是 AST 门禁）。
  这一轮就踩到了：`park_segments` 写在 `except BaseException` 里，门禁立刻报出来。

### 15.2 标签层级与颜色：层级是**前缀**，不是树表

要的东西看起来是"树"，但树表会带来一整套维护成本（父节点要不要存在、移动节点要不要
级联改名、删空节点谁来清）。实际需要的只是**分组**，而分组用名字里的 `/` 就够了：

| 视角 | 结论 |
| --- | --- |
| 用户 | `系列/角色A` —— 不用学任何东西 |
| 实现 | 一次带分隔符的前缀匹配（`rt.tag LIKE ? ESCAPE '\'`，转义符与 `%` 的语义分开） |
| 存储 | 零新表；`resource_tags.tag` 就是全路径 |

三个细节：

* **中间层要补齐**。父标签**未必**自己是个标签（用户可能只打过 `系列/角色A`）。
  界面渲染"有资源的节点"会缺一层，所以 `all_tags()` 按 `parent` 把中间层补成
  `n=0` 的节点，并给出 `n_tree`（连子标签一共多少）—— 那才是用户点下去会看到的数量。
* **前缀必须带分隔符**。`系列` 不许把 `系列二` 一起捞进来。这与 §10.2 的教训同一型：
  "看起来能用但结果多几条"是最难归因的那类偏差。
* **归一化阶段挡住退化形状**。尾随 `/` 让 `a/` 与 `a` 互为父子且树上多一个空节点；
  超过 `MAX_TAG_DEPTH` 说明用户把一整条路径粘了进来。两条都在 `normalize_tag` 里就报错。

**颜色**存在独立小表 `tag_meta(tag, color)`：颜色是**标签**的属性，不是资源的属性。
所以标签还没有任何资源也能先设色，资源全删了颜色也不跟着丢 —— 用户重新打上同一个标签
时它还在。清色 = **删行**，不留 `color=''` 的空壳（否则表会随着"点一下又取消"慢慢涨）。

⚠️ **调色板由后端下发**（`GET /library/tags` 回 `colors`）。和 `error_kind` 的中文标签
（第 9 条静默坑）是同一条理由：**界面上人看的文案与配色只能有一个来源**。前端只拿键去
查表；自己编一套色值的话，后端改了色界面还是旧的，而没人会想到去查"色值写在哪"。

### 15.3 DASH 字节区间与多时段

V35 把 `SegmentBase` 与多 `Period` **明确拒绝**了。拒绝是对的（硬下会得到"成功但没用"
的文件），但这两类在真实站点里是存在的，所以这一轮补上。

#### 字节区间：地址多了一种写法

分片地址从 `URL` 扩展成 `URL` **或** `(URL, (起, 止))`（闭区间，直接当 `Range` 头用）。
变化集中在两个点：

* `_seg_target(seg, headers)` —— 唯一知道"两种地址怎么变成一次请求"的地方。
  ⚠️ **带 `Range` 时绝不能同时带条件请求**：服务器对"条件 + Range"回的是 304 而不是 206，
  于是那一段字节永远取不到。与 `base.py` 里那条纪律（条件请求 ⊥ 续传）是同一件事。
* `fingerprint()` —— 序列化时必须把区间一起喂进哈希，否则"同一批分片、不同区间"会被
  判成同一批。

#### `sidx`：起点错了会整体错位

`SegmentBase` 有 `indexRange` 时，分片边界要从 `sidx`（Segment Index box）里解。
按 ISO/IEC 14496-12 §8.16.3：第一段的起点是 **sidx box 结束处 + `first_offset`**，
之后每段紧接着上一段。这不是"差不多"能对付的 —— 起点差一个 box 头就会把**每一片**
都切错位，拼出来是"长度对得上、能播、但每隔几秒糊一下"的东西。

所以这一条**拿真实编码器的字节对答案**：`test_features_v36.py` 里钉了一份 ffmpeg
真正吐出来的 sidx（52 字节，version=1，引用一段 13155 字节的媒体），断言
`parse_sidx(blob, 814) == [(866, 14020)]`。手造的 box 只能证明自洽，证明不了"我理解的
规范与编码器一致"。

三个"认不出就拒绝"：

* 服务器**忽略 `Range`**（状态 200 且长度对不上）→ 报错。拿整份文件从 0 开始解，
  分片边界会整体错位。
* `reference_type=1`（嵌套索引）→ 报错。展开它要先请求下一层索引，硬当成媒体段拼进去
  得到的是**索引的原始字节**。
* `SegmentBase` 没写 `BaseURL` → 报错。那时所谓"媒体文件"就是 MPD 自己的位置，请求它
  会下回一份 XML，而字节数是"有内容"的，长度校验发现不了。

⚠️ 还有一个只在"没有索引"时出现的坑：没有 `indexRange` 时**整个文件就是一段**，
而它**自带**初始化段（`ftyp`+`moov` 就在开头）。此时若把**同一个文件**的
`Initialization` 也拼到前面，等于把 `moov` 放两遍 —— 播放器遇到第二个 `moov` 的行为
不确定，而字节数"看起来没问题"。判据按**目标文件**比，不按"有没有区间"：
`sourceURL` 指向另一个文件的初始化段是必须留下的。

#### 多时段：逐时段下，**逐轨**拼

每个 `Period` 各有自己的 `moov`，所以：

* **不能字节拼接** —— 那是坏文件。要交给 ffmpeg 重新封一遍（`-f concat -c copy`）。
* **必须逐轨拼** —— 音视频混着拼会得到音画错位。

⚠️ 顶层 `video`/`audio` 在多时段时**必须是 `None`**。这是**故意**的：原来"只读第一条轨"
的调用方拿到空值后会自己炸，而不是静默少下后面全部。用一个"方便字段"换来静默截断，
是这次特意拒绝的取舍。

另外 `_ffmpeg_concat` 写 concat 清单时**要转义单引号**：相册名里出现 `'` 会让 demuxer
读到半个路径，报"文件不存在"，而文件明明在那里。

### 15.4 顺带修掉：HLS 路径丢掉调用方的 `info`

`_download_m3u8` 里预检结果的局部变量与**参数**重名（`info`），把任务管理器传进来的
那只字典直接顶掉了 —— 于是 HLS 视频的 `resolved_url` 一直是空的（DASH 与直链都正常）。
不报错、不影响下载，只是那条溯源信息静静地没了。

这是"形参被局部变量遮蔽"这一类，靠读代码很难看出来，靠**在测试里断言那只字典被回填**
才炸得出来（`test_hls_fills_the_callers_info_dict`）。

### 15.5 文档与测试复核

**关键词扫描**（`暂不支持 / 还没有 / 只放内存 / 未实现 / 缺一个 / 明确拒绝`）照例跑了一遍。
与 V35 不同，这一轮**没有发现"谎报能力缺失"** —— V35 列的三项确实还没做（这版才做完），
当时写的边界描述是准的。需要改的只是**能力描述本身过期**，共四处
（`SegmentBase` / 多 `Period` / 扁平标签 / 「暂存区只收单文件」）。

> 教训：复核的价值不只是"抓谎"，也包括**确认没谎**。两次都得出同一个结论就停止扫描，
> 下次就会开始凭印象增删条目 —— 那正是 V35 那两处谎报的成因。

**旧断言的前提失效**（第 C 类）这一轮出现三处，都改了断言而不是改产品：

| 断言 | 原来守什么 | 现在怎么守 |
| --- | --- | --- |
| `all_tags() == [{"tag": ..., "n": ...}]` | 只统计 `done` 资源 | 改成断言**字段的值**（`n`），不再跟整只字典比 —— 加一个键就红，红的是断言太死 |
| `test_segment_base_is_refused` | `SegmentBase` 必须被拒 | 现在支持了；但**原来要守的那件事没变**（不许下出"成功但没用"的东西），换成断言新的拒绝判据：没写 `BaseURL` |
| `test_multi_period_is_refused` | 多时段必须被拒 | 同上，换成断言"多时段时顶层 `video`/`audio` 是 `None`" |

⚠️ 前两条特别值得记：**它们改之前是"通过的"**。`test_segment_base_is_refused` 之所以
还在过，是因为新代码在另一条判据上仍然报错（没有 `BaseURL`），报错文本里恰好有
`SegmentBase`；`test_multi_period_is_refused` 是因为新 fixture 没声明时长，
先在时长那一步就炸了。**"通过但理由已经不对"比失败更危险** —— 失败会逼你看，
通过不会。
...
| 文件 | 性质 |
| --- | --- |
| `backend/core/partials.py` | 改：分片缓存（`kind=segments`）、`seg_dir` / `fingerprint` / `park_segments` / `take_segments` / `ensure_fingerprint`、`sweep` 自愈孤儿、`last_error` 留痕 |
| `backend/downloaders/dash.py` | 改：`parse_sidx` / `_range_of` / `_has_base` / `_segments_from_base`、`SegmentList@mediaRange`、多 `Period`（返回 `periods`） |
| `backend/downloaders/video.py` | 改：`_seg_target`（Range 寻址）、`_resolve_index`、`_ffmpeg_concat`、多时段逐轨拼接、HLS `info` 遮蔽修复 |
| `backend/core/database.py` | 改：`TAG_SEP` / `TAG_COLORS` / `tag_parent` / `tag_depth` / `normalize_color`、`tag_meta` 表、`all_tags` 补中间层与 `n_tree`、`library_filters` 加 `tag_children` |
| `backend/api/tasks.py` | 改：`/library/tags` 下发 `colors`/`sep`、新增 `POST /library/tags/color`、`/library` 加 `tag_children` |
| `backend/models/schemas.py` | 改：`TagColorIn` / `TagColorOut` |
| `frontend/src/api.js` | 改：`setTagColor`、`listLibrary` 支持 `tag_children` |
| `frontend/src/components/LibraryPanel.vue` | 改：标签树（缩进 + 末段名）、颜色圆点、调色板弹层、「含子标签」开关 |
| `frontend/src/App.vue` | 改：暂存区占用里区分「单文件 / 分片缓存」 |
...
## 18. 新站点探针（`scripts/probe_site.py`）(2026-09-23)

### 18.1 先算清成本，再决定做什么

用户问「你推荐更多站点插件，怎么做」。这个问题值得先算账，因为直觉会答错 ——
直觉是「加站很费劲」，而实际成本分布完全不是这样：

| 环节 | 实测成本 |
| --- | --- |
| 写声明 | `collectors/xchina/gallery.py` 184 行，其中 96 行是实测文档；**声明本体约 70 行** |
| 改基类 | **零**。接 pexels（第二个实例）时没有改动 `gallery_base` 任何逻辑 |
| 探测站点行为 | 五项，此前**只能手搓 curl**，没有工具 |

`gallery_base`（1636 行）已经把枚举 / 存在性判定 / 限速 / 镜像降级 / 命名 / 预览全部
兜住，所以「再加一个站」的边际成本 ≈ 一份常量 + 一组 `id_samples`。**不值钱。**

真正的成本在探测，而且它的失败模式很坏 —— 其中几项猜错都是**静默**的：

* ① 存在性判定猜错：站点对不存在的图集照样答 `200 text/html`，于是枚举「正常地」
  采到 0 个资源，任务还报 success（2026-09-18 事故）。
* ⑤ URL 形态漏一种：把**页码**当 ID，去枚举不存在的图集 —— 又是「成功但 0 资源」。
* ② 请求头猜错：「浏览器能看、代码全 403」，极易被误判成防盗链或需要登录。

所以结论是：**不要广撒网批量加站**，一次一个、探针先行；而唯一值得先补的基础设施
就是那个不存在的探针。

> 区分一下：`scripts/probe.py` 是**浏览器**探针（跑 `BrowserCollector` 看四种解析器
> 发现了什么），服务的是通用采集器，与这五项无关。名字像，职责完全不同。
...
至少给一条**资源直链**：它把基址与序号补零宽度直接写在 URL 里，是手上最硬的一份
证据（用户右键复制到的那条）。输出分两半：五项**实测报告** + 一份可直接填的
`GallerySite` 草稿（`--out` 落盘）。

| 段 | 怎么测 | 结论长什么样 |
| --- | --- | --- |
| ① 存在性 | 探给定直链那个序号（应存在）+ 一个基本肯定越界的序号（默认 9000），**两端对照** | 「越界仍是 200 但类型不是 image/* → 只能用 Content-Type 判定」 |
| ② 请求头 | 固定 UA，只改 `Accept`：不给 / `*/*` / `image/*` 三档，**一次只变一个变量** | 「被校验：有的一律 403、有的 200」 |
| ③ 页面 | 一次 GET；403 / 挑战页判为「页面走不动」，并说明**不影响采集** | 顺带收割页面里**真正出现**的资源直链（绝对 + 根相对两种写法都收） |
| ④ 变体 | 挨个试候选后缀，**只有真的回 200 + 媒体类型才算档位** | 「3 条线路」→ 写进 `variants`，其余自动成为 `mirrors` |
| ⑤ URL 形态 | 按 ①③ 的证据生成正则草稿，再**套回用户给的每条 URL 自检** | 「自检：4/4 条输入解析出一致的 gid」+ 页码是否被识别 |

### 18.3 两条「不许编结论」的守卫

这是本轮**自己踩出来的**，也是这个脚本最值钱的部分 —— 它验的是**探针自己的输出
口径**，不是站点：

1. **一个存在的序号都没探到 → 拒绝下结论。** 第一版把「越界返回 403」判成
   「状态码可用」。而当时的真相是**整个站都进不去**，那条结论会把一个完全不可达的
   站点写成「判定规则已确认」。现在这种情形直接输出「判定规则**还没验出来**，不要
   照抄下面的结论」，并给出可行动的下一步（`--impersonate chrome` / `--proxy`）。
2. **「空洞」必须与「末尾缺失」分开。** `--scan 6` 探一个 5 张的相册必然触发一次
   末尾缺失；把它报成「序号可能不连续」的结果是这行永远在闪、没人再看它。真正的
   危险是**中间**有洞：指数探上界会把上界定在缺口之前 —— 那是「300 张只采到 4 张」
   的静默截断。现在只报「缺失之后**又**有命中」的那种。

### 18.4 靶子为什么是本地假站点，不是 xchina

本来最好的靶子是 `collectors/xchina/gallery.py` 模块文档里钉死的那五项结论
（越界 `200 text/html` / `Accept` 必须带 `image/*` / HEAD 可靠 / 资源分到
`photos`、`photos2` / 7 种 URL 形态），每条都写了日期。**但它过期了**：

```
2026-09-23 复测: img.xchina.io 对任何请求都返回 403, 且是 text/plain
(不是 CF 挑战页); 裸 curl 带浏览器 UA + image/* 也一样。
也就是说这台机器/这条网络上整段不可达, 与 Accept、TLS 指纹、HEAD 都无关。
```

拿它当靶子只会得到「探针坏了」的**假警报** —— 而这正是本项目最爱犯的错：把环境
问题当成回归。所以把同样五条行为做进 `tests/test_probe_site.py` 里的一个本地 HTTP
服务：`_FakeSite` **刻意不返回 404**，越界与未登记的档位一律 `200 + text/html`；
Accept 不含 `image/` / `video/` 就 403（**注意 `*/*` 不含 `image/`** —— 按「含通配
即放行」写会把这条行为整个抹掉）；只认三条档位；页面里同时放一条绝对 URL 和一条
根相对路径。

这样结论就与网络无关了。顺带钉住的两条针对性回归：

* **正则必须按路径段对齐。** 生成相册页正则时若取「gid 前 N 个字符」，窗口取 8 而
  `/photo/id-` 是 10 个字符，前缀就被截成 `hoto/id-`。它**照样能匹配**（子串
  `search`），所以跑起来一切正常 —— 直到换一个路径深度或遇到 `notphoto/id-` 这种词。
  用例直接断言它长什么样，**并且不匹配 `notphoto/id-`**。
* **基址末尾无条件补 `\d*`。** 采信的只是**一条**直链，而站点可能把相册分到
  `photos` / `photos2` / `photos3`。用户恰好粘了 `photos` 那张，不代表别的相册也在
  那儿；只认默认桶的后果是 `photos2` 的直链落给通用采集器 —— 预览报错而采集本身
  好使，典型「一半好一半坏」。这一层**必须比用户给的那一条更宽**。

### 18.5 验证（2026-09-23）

| 项 | 结果 |
| --- | --- |
| `tests/test_probe_site.py` | **新增 12 项**，全绿（含一次端到端：起假站点 → 跑脚本 → 断言五段结论 + 草稿 `ast.parse`） |
| 探针生成的 xchina 草稿 | 与真实声明**逐项对上**：`base` / `seq_format` / `gid_shape` / `page_tail` / URL 形态 |
| 本机 `curl-cffi` | 补装（项目主依赖）。此前它缺失导致 4 条测试红，属**环境问题不是回归** |

### 18.6 新增/改动文件（本期）

| 文件 | 性质 |
| --- | --- |
| `scripts/probe_site.py` | **新增**：五项探测 + 草稿生成 + 两条「不许编结论」守卫 |
| `tests/test_probe_site.py` | **新增**：本地假站点 + 12 项（口径 / 守卫 / 正则两处回归） |
| `README.md` | 改：「接入新站点」一节（五件事表 + 用法）、脚本清单、区分两个 probe |

### 18.7 真实站点实测（2026-09-24）：五处「看着能用」

18.4 说「回归靶子用本地假站点」，那是对的；但**只**用本地假站点会漏掉一整类问题 ——
假站点验的是「我**想到**的行为」，而真实站点的 URL 形态是**想不出来**的。

于是换一批**形态怪**的真实站点跑探针（都合法公开：数字图书馆 / 在线漫画 / 图库），
当场暴露五处「产出错误」。共同点：**都不是崩溃** —— 探针跑得通、报告看着也像回事，
错的是**它给出的结论**。所以「跑得通」发现不了这类问题。

| # | 探针原本会做什么 | 真实症状 | 修法 |
| --- | --- | --- | --- |
| 1 | 把序号之后的**高熵内容哈希**当成「画质后缀」 | MangaDex 的 `/data/<hash32>/1-<sha256>.png`：草稿写成「第 2 页 = `2-<第 1 页的 sha256>.png`」→ 必然 404。而它**自己已经测出来了**（`seq 1` 命中、`seq 2..6` 全不命中、`本次命中 1/6`），却照样把那份草稿打出来 | 新增 `suffix_is_opaque()`（长度 ≥40 或含 ≥16 位连续 hex）→ **拒绝出草稿**，exit 2 |
| 2 | 给 ⑤ 自检**没通过**的 URL 编一个期望 gid | 报告里明明写着 `页面URL -> None`，草稿的 `id_samples` 却给它配了采信直链的 gid。**这是编证据**：`check_site()` 会拿着它空转，而人看不出哪里不对 | ⑤ 返回逐条解析结果，`id_samples` **只写自检通过的**；没通过的以注释列出 |
| 3 | 把末尾**整段是数字**的路径当「相册桶号」 | picsum 的 `/id/1040/200/300.jpg` 推出 `base=/id/1040`，`split_digit_base` 砍成 `/id/` + `base_candidate_digits=1040` → 基类去展开一千多个候选 | 数字紧跟在 `/` 之后（整段）就不当桶号；桶号 > 99 也不认 |
| 4 | 多段 base_path 去掉末尾数字后留下尾随 `/` | 生成 `/id//(\d{4,})/...` —— 一条**几乎匹配不上任何真实 URL** 的正则，但看着「像是写好了」 | `rstrip("/")` |
| 5 | 命中 **0** 个序号时照样打印草稿 | 那份声明的每一条都是「按 URL 形态猜的」，没有任何实测支持；而人很可能只翻到「GallerySite 草稿」这一段 | 草稿顶部加强横幅；命中 **1** 个另有一种**歧义**提示 |

第 1 条还有一个「口径」层面的教训：**探针的 ① 与草稿在自相矛盾**。① 用实测说
「只有第 1 个序号存在」，草稿却说「改序号就能枚举」。两句话出自同一个程序，而
**照抄的那一份是错的**。所以现在：命中 1 个 → 明说这是**歧义**（①图集本来只有 1 张
②每页文件名各不相同 ③序号不从 1 开始），并给出分辨方法（**再给一条不同序号的直链**）；
命中 0 个 → 草稿顶部标「没有任何证据支持」。

**为什么判据取这么宽**：`suffix_is_opaque` 的阈值（≥40 字符 / ≥16 位连续 hex）是刻意
宽松的。漏判成「不透明」最多让人多看一眼、多跑一次；判反了则会生成一份「任务 success
但 0 资源」的声明 —— 按**误报的代价**决定判据松紧，这里宁可误报。

**顺带记下的环境事实**（别再拿它们当靶子）：漫画柜 `manhuagui.com` 连接超时；
Lorem Picsum 的图片直链要 `hmac` 签名（裸 URL 一律 400）；Internet Archive 的页图是
`BookReaderImages.php?zip=...&file=..._0001.jp2`，连「这是资源直链」都判不出来
（末段没有媒体扩展名）→ 探针按「页面 URL」处理，exit 1。

### 18.8 验证（2026-09-24 续）

| 项 | 结果 |
| --- | --- |
| `tests/test_probe_site.py` | **12 → 19 项**，全绿。新增的 7 项全部指向 18.7 的五处（含两条端到端：不透明后缀站 exit 2 + 不生成草稿；命中 0/1 时的横幅与歧义提示） |
| 真实站点复跑 | MangaDex 现在 exit 2 且报告里带歧义提示；picsum 的 `base` 不再被砍、正则不再有 `//` |
| 全量 pytest | **1082 用例全绿**（1075 + 7） |

### 18.9 把「五处补丁」升级成一条门禁（2026-09-24 三）

18.7 那五处是逐个手写 `if` 堵的。堵完发现它们只是**同一类错**（探测结果没进结论）的
五个化身 —— 于是把模式抽了出来，理由是：**补丁只能挡住已经见过的那几种形态**。

分档判据就一句 —— **猜错了，是每条 URL 都错，还是只是少采一点**：

| 档 | 字段 | 门禁 |
| --- | --- | --- |
| A | `base` `seq_format` `suffix` `gid_shape` `id_patterns` | 缺实测证据 → `refuse()`，exit 2 |
| B | `variants` `quality_map` `id_samples` | 照出，但顶部汇总标注 |
| C | `input_forms` `album_url_template` | 只标注 |

实现上是 `build_draft()` 返回 `Draft`（**`str` 子类**）外加 `gaps`；`Draft.blocking` /
`.advisory` 就是门禁的输入。继承 `str` 是刻意的：既有调用方（含测试）写的是
`draft.split(...)` / `x in draft`，不该为一个新字段把它们全改掉。

**另外两处前移**：

- `probe_reachability()`（第 0 段）：直链不通就早退，省掉后面四项二十来个请求。判据是
  **200 + 媒体类型**，不是"状态码 200" —— 站点对拿不到的图集照样答 `200`，只是
  Content-Type 变成 `text/html`（这正是 18.7 第 5 条的来源）。`403/429/503` 且用户没
  显式指定指纹时，**自动**用 `chrome` 重试一次再判定。
- `validate_draft()` + `report_draft_selfcheck()`：草稿拼完就地 `exec`（把相对 import
  换成绝对 import 即可，`backend` 已在 `sys.path`）并跑 `check_site()` —— 它是这份声明
  **唯一**的验收标准。以前靠人肉（存盘 → import → `selfcheck.py`）。
  ⚠️ 自检结果**不阻断**出草稿：草稿是给人改的起点，不是交付物。

**目视输出时又抓出两处**（都不在计划里 —— 是把完整输出打出来看才发现，测试不会报）：
...
- `short()` 从头截断，把 `/10.html` 这个**关键末段**砍掉了，于是"末段是页码"那条提示里
  两个 URL 长得一模一样，人分不清探针在说哪一条（而这两个 URL 的处理规则**相反**）。
  → 新增 `short_tail()`（保尾），用在末段提示、自检失败清单、`id_samples` 漏项注释三处。

### 18.10 验证（2026-09-24 三）

| 项 | 结果 |
| --- | --- |
| `tests/test_probe_site.py` | **19 → 23 项**，全绿。改写 2 项（前提变了：直链不通现在在第 0 段早退 / 命中 0 现在走档 A 拒绝），新增 4 项（`Draft` 分档、`validate_draft` 两路、`short_tail`），主用例加 2 条断言（草稿自检通过、正则不重复） |
...
## 21. 仓库门禁: 让「GitHub 红」与「假绿」不再可能（2026-09-24 五）

### 21.1 要治的是四种状态, 不是一个"红"

问"怎么才能不红"之前得先分清红/绿各自有几种。它们**看起来一样, 根因完全不同**:

| 状态 | 表现 | 根因 | 治法 |
| --- | --- | --- | --- |
| **真红** | CI 报一条具体失败 | 代码确实坏了 | 修代码 |
| **假红** | CI 报一条失败, 但代码是对的 | 判据挂在**文案**或**未承诺的顺序**上 | 判据改挂代号/结构 |
| **假绿** | CI 绿, 但**该跑的没跑** | 闸空转、依赖没装、用例被跳过 | 报 `checked`; `0` 算红; 跳过分档 |
| **没接线** | 守卫写好了, 谁也没调 | 新加的脚本没进 CI | R-CI-1 |

这一轮把四条都落成了**程序可执行的规则**, 落点如下。

### 21.2 新增/改动

| 文件 | 作用 |
| --- | --- |
| `scripts/gate.py`（新） | `Gate` / `Problem` / `NOTHING_CHECKED` 的**唯一定义** —— 判据形状有两份就会出现"这边认 `checked`、那边不认", 而那种不一致只会让某道闸永远绿 |
| `scripts/gateguard.py`（新） | 六道仓库级检查: CI 契约 / 文档数字 / 未用 import / Markdown 围栏 / `with` AST 门禁 / 行尾 |
| `.gitattributes`（新） | **行尾规则的唯一来源**(`* text=auto eol=lf`); 门禁直接读它, 不另写一份清单 |
| `tests/test_gateguard.py`（新） | 76 项: 每道闸都有"故意造坏"的输入, 加上真实仓库上的 5 条 |
| `tests/test_skip_policy.py`（新） | 跳过策略的自检(纯函数双向 + 全仓静态扫) |
| `tests/conftest.py` | R-CI-3 的运行时钩子 + 静态标签要求 |
| `.github/workflows/ci.yml` | 顶层 `UWC_CI=1`; backend 装 ffmpeg、跑 gateguard/selfcheck/add_site/verify_output; frontend 从 `npm install` 改成 `npm ci` 并接上 `test:task-query` |
| `Makefile` | `make guard`; `make test` 带上门禁 |
| `README.md` | 「CI 契约」章节: 13 行契约表 + R-CI-1/2/3 |

三条规则(README 里也写了一份, 那是"给人看的落点"):

* **R-CI-1 没有第三态**: 每个脚本要么在 `ci.yml` 的调用链里, 要么在契约表里写明"只在本地跑 + 为什么"。两者由 `gateguard` 逐条核对 —— 它**解析 YAML 结构**取 `run:` 与 `env:`, 顺着 `npm run <name>` 解出 `frontend/scripts/*.mjs`。
* **R-CI-2 数字必须可核对**: README 是唯一的"当前口径"落点; `docs/` 里的版本史数字**不参与核对**(那是"当时是多少", 拿今天实测去比是另一种假红)。
* **R-CI-3 平台门控之外不许跳过**: 跳过必须带 `[platform:…]` / `[deps:…]` / `[env:…]`; `[deps:*]` / `[env:*]` 在 CI 里一律不许出现。

### 21.3 ⚠️ 三条错是**端到端**跑出来的, 单测当时全是绿的

这一节是本轮最值得记的部分: 三个错都不是"新代码写错了", 而是**判据自己的问题**,
而且三个都由同一次全量(以及随后的逐条复盘)抓出。

**一、`[platform:X]` 的判据方向反了 —— 那条规则永远不可能两边都绿。**

原判据: "标签必须**等于**当前平台"。听起来像"验证 tag 说的是真的", 实际上:

* 仓库里同时有"只在 POSIX 跑"和"只在 Windows 跑"的用例;
* 在 Windows 上, 前者会跳, 标签是 `[platform:posix]` → 判红;
* 把它们改成 `[platform:windows]` 就能过 Windows, 但同一个源文件拿到 POSIX 上,
  Windows-only 那条跳了、标签 `[platform:windows]` → 又红。

**无论标签怎么写, 总有一个平台是红的。** 正确方向是"X = 这条用例**需要**哪个平台",
于是合理的跳过必然发生在**别的**平台上, 判据是 `X != 当前平台`。

⚠️ 更要紧的是**它当时没红**: 那轮只跑了 `test_skip_policy.py`, 而那几条双向用例是照
**实现**写的(实现说"要等于当前平台", 用例就断言这一条), 于是"实现与意图一致地错"通过了
自检。抓住它的是端到端那次全量 —— 真实站点上真的跳了, 标签真的不等于当前平台。这就是
第 20 条坑同族的 F 类("通过但理由已经不对"), 也是 ⑦ 那条心法的用法: **单测可以忠实地编码
一个错的意图**, 所以新判据必须被整条链路跑到一次。那个不可能性证明被写成了用例:
`test_the_platform_tag_is_satisfiable_on_both_platforms`。

**二、一条拿墙钟当判据的用例。**

`test_proxy_breaker_cooldown_expires` 原来写 `cooldown=0.01` + `sleep(0.05)`, 看着很保险。
但 `note_failure()` 在写完内存状态之后**还会落一次代理健康 JSON**, 那次文件写在负载下
超过 10ms, 于是后面的"熔断中"断言看到的已经是到期状态。报出来的是
`assert 'http://a:1' == 'http://b:2'` —— 完全看不出是竞态。
改成 `cooldown=60` + `_is_blocked(now=…)` 显式推进时刻: 判据不挂在"此刻几点"上。

**三、README 的用例数落后于实测。**

这条是 R-CI-2 自己抓的, 属于规则按设计工作。

### 21.4 ⚠️ 两个"看起来在工作"的假绿路径

* **`git ls-files` 在非仓库顶层的目录里会成功地返回空清单。** 本项目的项目目录位于父仓库
  `C:\Users\admin` 之下, 于是 `tracked_files(root)` 拿到 `[]` —— 门禁"一项都没核到"却
  表面全绿。修法: 先比 `git rev-parse --show-toplevel` 与 `root`, 不匹配就走 `os.walk`。
  (这是本项目第 19 条坑的远亲: **静默返回空**比报错危险得多。)
* **注释里提到 `npm install` 会被判成"CI 用错了命令"。** 第一版在整份 ci.yml 文本上扫,
  于是那句"用 `npm ci` 而不是 `npm install`"的注释把一条**正确**的配置判红了 ——
  典型假红。修法: 用 `yaml.safe_load` 解析结构, CI 纪律只在 `run:` 与 `env:` 上判。
  与 20.5 的 AST 门禁是同一条道理: **判据要落在语法结构上, 不要落在文本上。**

另外顺手修掉 12 处**真·未用 import**(逐个核对"这个名字在整份文件的其余文本里一次都不出现"),
以及 5 个整份 CRLF 的历史文件(`Path.write_text()` 在 Windows 上默认把 `\n` 翻成 `\r\n`,
一次读-改-写往返就把整份文件换掉)。未用 import 的判据刻意**保守**: 误报的代价不对称 ——
漏报只是留一个多余 import, 误报会让下一个人**删掉一个在用的 import**。

### 21.5 验证

| 项 | 结果 |
| --- | --- |
| `tests/test_gateguard.py` | **76 项** |
| `tests/test_skip_policy.py` | **13 项**(含静态扫全仓跳过点) |
| 跳过策略实测 | 造探针文件: 本地未打标签 → `exit=1` 并印终端汇总; `UWC_CI=1` 下 `[deps:ffmpeg]` 也 → `exit=1`; 探针已删 |
| 全量 pytest | 见 §22.6（同一个数） |

**性能备注**: `gateguard.collected_case_count()` 会起一次 `pytest --collect-only` 子进程
(十几到几十秒)。`tests/test_gateguard.py` 里有三条用例各自把全部闸在真实仓库上跑一遍,
所以那个结果**必须缓存**(`functools.lru_cache`), 否则一趟全量白等一两分钟。

---

## 22. 本地相册集: 把用户自己的目录当相册集（2026-09-24 五）

### 22.1 它补的是哪一块

采集侧解决"网上有什么、怎么搬下来"; 但用户手上早就有一堆**已经在本地**的照片。
「本地相册集」把**用户显式登记过的目录**当成相册集: 相册网格 + 相册内照片列表 +
一个**跨相册的随机池**(图集站给不了这个 —— 随机抽的是自己忘了的照片)。

`相册 = 直接装着照片的目录`。这个定义唯一不需要额外配置、也不需要猜: 换成"递归收集子孙
目录"会让 `2024/` 这个相册里混进 `2024/旅行/` 的内容, 于是"这个相册有多少张"永远说不清;
换成"只看下一层"又会让 `照片/2024/旅行/` 这种真实结构整片消失。层数可调(`local_album_depth`),
但**口径只有一条**。

### 22.2 只读边界 —— 而且它是**结构化**判据

用户登记的是他自己的目录, 所以这个模块**绝不写、改、删根内的任何文件**; 唯一落盘的是
缩略图缓存, 而它在**我们自己的数据目录**里(`<库文件同级>/local_albums/_meta/thumb/`,
复用 `core/thumbs.py` 那一份实现)。

只读靠两层判据, 而不是一句注释:

1. 运行时: 跑完一圈读路径(列相册 / 列照片 / 随机 / 收藏 / 扫描 / 清缓存)之后, 根的
   **逐文件 size + mtime_ns 指纹**必须一模一样。比 mtime 而不是"文件还在不在":
   往用户的照片上追加一个字节或 `touch` 一下都会让相册软件的排序变掉 —— 那已经是"改了
   用户的东西"。
2. **结构化**: 直接 AST 扫 `core/localalbums.py` 里有没有任何"写磁盘"的调用。
   `Path.replace` 刻意**不在**黑名单里(`rel.replace("\\", "/")` 是路径归一, 与落盘无关,
   而 AST 分不出接收者是 str 还是 Path) —— 误报的代价是"下一个人为了让它绿而删掉一行
   正确的代码", 所以那个盲区交给上面那条运行时指纹兜底。

配套的两条语义也写死在代码与界面里: **"取消登记"只删那条记录**(二次确认里明说"文件一个
都不会动"), **收藏只写我们自己的库**(不往用户的相册目录里塞 `.favorite` 之类的标记文件 ——
那会污染目录, 而且用户一旦把它当垃圾清掉, 数据就没了)。

### 22.3 出图接口只收 `(root_id, rel)`, 从不收裸的绝对路径

收绝对路径的接口就是给"任意文件读取"开门, 剩下唯一的挡板只有一句"请传合法路径" ——
而那句话不在代码里。信任锚只有一个: **用户显式登记过的根**。所以 `safe_photo` 的三道判据是
`resolve()` 之后在不在根内 → 是不是真实文件 → 是不是图片扩展名。

* 判据是"归一之后在哪", **不是"字符串里有没有 `..`"**: 后者会拒掉
  `旅行/../旅行/a.jpg` 这种完全合法的路径, 而"为了安全把人挡在门外"的错误最难被发现
  (用户只会觉得"有时打不开")。
* `not-image` 回 **415** 而不是 404: 让"调用方传错了"和"文件真的没了"在日志里不一样。
* 对称的一条结构化判据: 扫 `api/local.py` 的函数签名里有没有 `path` / `file` 之类的参数 ——
  **没有任何接口接受绝对路径**(唯一的例外是"删掉一条失效收藏", 它只删我们库里的一行,
  连 `Path()` 都不碰, 所以输入风险只是"删错一行收藏")。

### 22.4 两套口径: 打开相册实时, 随机池用快照

* **打开某个相册 = 实时列目录。** 用户看到的就是磁盘上此刻的内容, 所以看得见的一定打得开。
* **随机池 = 索引快照。** 一个 5 万张的库每次随机都重扫一遍是不合理的。

两者能同时成立, 靠的是"**打开相册这一步会把索引里那一条就地修正**"(`_patch_album`)。
不这么做, 一个刚被删掉一半照片的相册会继续在随机池里出现那些不存在的照片, 表现是
"随机里点开是坏的" —— 极难怀疑到缓存上。剩下的偏差被 `local_album_ttl`(默认 30s)封顶,
而且接口会返回 `at`(快照时刻)与 `pool`(池里多少张), **界面必须显示** ——
否则"怎么没抽到我刚放进去的那张"没有任何解释入口。

### 22.5 随机池: 按轮发牌

`seed` 与 `page` 是两个不同的东西: 「换一批」换 seed(整副牌重洗), 「更多」加 page(翻下一页)。
同一 `seed` 下各页**不重叠、也不缺项**(整副牌恰好覆盖池里每一张)。少了任何一条, 用户看到
的就是"有些照片永远刷不到", 而那是**看不见**的 bug。

`mode="album"`(默认)**按轮发牌**: 每轮从每个相册各取一张。这样 3 张的小相册与 3000 张的
大相册在牌面上出现频率接近, 不会被整个淹掉; `mode="photo"` 是整池洗牌, 大相册按张数占优。
两种取向都留着 —— 而它们的差别被写成了**确定性**的用例(取 2 张时前者必然两个相册各一张)。

### 22.6 ⚠️ 抓出的三个坑

1. **`_list_album` 的条目不带 `root_id`, 于是收藏标记全部落空。**
   `_with_favorite` 用 `item["root_id"]` 反查根路径, 而 `_list_album` 只产出
   `name/rel/album/size/mtime` —— `roots_by_id.get(None)` 恒为 None, 于是**每一张的
   `favorite` 都是 False**。不报错, 只是星号永远不亮。修法: 让 `_list_album` 收 `root_id`
   并写进每一条(上层就不必"自己再拼一遍", 而那正是这一层不该做的事)。
2. **就地给缓存里的 dict 加键 = 把一次查询的状态永久写进缓存。**
   `photos()` 返回的那些 dict 就是**索引缓存里的同一批对象**。第一版直接 `p["favorite"] = …`,
   于是取消收藏之后界面还是老样子 —— 而"改了没反应"是最难被归因的一类现象。
   修法: `_with_favorite` **返回副本**; 并加一条用例钉住"缓存里那份是干净的"。
3. **`favorites` 的失效原因要分开, 不许合成一句中文。**
   "文件没了"、"那个目录已经不再被登记"、"不是图片"是三种完全不同的排查方向。所以
   `stale` 里带的是**原因代号**, 界面按代号分支; 同时它们**不许被静默过滤** ——
   少了东西却不说, 用户只会以为收藏功能坏了。顺带补上"删掉这条失效收藏"的入口:
   一个永远清不掉的失效记录比没有收藏功能更难看。
...
做完 V40 之后去看了同类产品。**先看清楚谁跟我们是同一件事**, 否则照抄会抄到
完全不同的约束上去:

| 产品 | 与我们的关系 | 吸收了什么 | 明确不做 |
| --- | --- | --- | --- |
| **Immich 的 External Library** | **同一个契约**: 把现有目录以 `:ro` 挂进来、只建索引、一个字节都不动 | 排除模式(glob) / 往年今日(Memories) / "文件消失先标 offline 再清理" | 人脸聚类、CLIP 语义搜索、地图(合规 + 重依赖) |
| **PhotoPrism** | 索引 `originals` + 端上分类 | 重复与相似检测(**复用项目已有的 `core/phash`**) | 按相机/颜色/地点多维筛选、Moments 日历 |
| **Eagle / Billfish** | 本地素材管理, 与"随机墙 + 收藏"的形态最近 | 重复检测、键盘快捷键 | 颜色搜索、智能文件夹、评分(都要一套新 UI 语义) |
| **Google Photos**(参照物) | —— | "On This Day" 是被提到次数最多的一个功能 | 语义搜索 |

四个能力各自的**设计纪律**, 三条都指向同一件事 —— **不许出现新的假绿**:

1. **排除规则必须报"跳过了多少"**(`index["excluded"]`)。
   一条写错的 glob **什么都不排除**, 而那个结果和"没有配置规则"长得一模一样。
   所以界面上的依据不是"照片少了几张", 而是这个计数 —— 它同时也是"规则生效了"
   的唯一证据。匹配用了三条判据(完整相对路径 / 末段文件名 / 任一祖先目录):
   严格的 glob 语义会让用户随手写的 `Raw` 静默失效, 而"规则静静地失效"是本功能
   唯一真正的风险。
2. **查重复在没有 ffmpeg 时返回 `reason="no-decoder"`, 而不是空列表。**
   这是四个能力里最要紧的一条: 空列表会被读成"没有重复", 而真实情况是"一张
   都没算"。与 `core/phash` 的 `NO_DECODER` / `DECODE_FAILED` 同源 —— 那两个值
   当初就是为了防止"把环境缺失当成文件坏"。
   另外**只标记不删**, 且单次有上限并报 `skipped`(5000 张各起一次子进程的请求
   只会超时, 而超时的后果是用户以为功能坏了)。
3. **扫描对账的 `None` 与 `{"added": 0, "removed": 0}` 是两个结果。**
   合并成 0 的话界面永远显示"没有变化", 而"这次其实没有可比对象"这个事实被抹掉。
   这正是 Immich 那条"先标 offline 再清理"要解决的: 用户得能看出**有东西不见了**,
   而不只是看到一个总数变小。
4. **往年今日的口径要随数据一起交出去**(`basis="mtime"`)。
   它是**文件修改时间**, 不是拍摄时间 —— 拍照片的 EXIF 要新增依赖, 而大量下载来
   的图片根本没有 EXIF。代价必须说清: 复制/重新导出会刷新 mtime, 于是整批导入的
   照片会挤在同一天。宁可少说, 不可说错 —— 所以界面上写的是"修改时间"。
   同一天反复调用给**同一批**(用当天日期做种子), 因为每次都换一批就没有回忆了。

### ⚠️ 顺带抓出并修掉的三个真 bug

前两个是"写的时候没想到", 第三个是**只有真图跑出来才看得见**的:

* `exclude_report` 里"声明条数"和"生效条数"**一起被截断**, 于是 `dropped` 恒为 0 ——
  一个"看起来没坏"的静默失败, 被那条"配了 57 条就该报丢了 7 条"的用例抓出来。
  修法是让解析逻辑只有一份(`_split_exclude`), 截断只发生在 `parse_exclude` 里。
* `PATCH /local/roots/{id}` 无条件调 `rename_root`, 于是"只提交排除模式"会顺手
  把名字抹掉。`name is None`(不改)与 `name == ""`(清空)必须分开。
* **`reason` 把"文件坏"说成了"机器没装 ffmpeg"。** 第一版写成
  `reason = "no-decoder" if not hashes`, 于是"相册里全是解不开的图"(每张都
  `DECODE_FAILED`)会被报成"本机没有 ffmpeg"。后果很具体: 界面照着 `no-decoder`
  提示"请安装 ffmpeg", 而用户明明装了 —— 于是他再也不信这个提示, **真正的环境
  缺失也就一起被淹掉了**。修法是 `no_decoder` 与 `undecodable` **分开计数**,
  而且 reason 只由前者决定。

### ⚠️ 冒烟时撞到的一个假阳性: 纯色图彼此"全是重复"

用 ffmpeg 造真图跑了一遍: 一张纯红、一张纯红、一张纯蓝, 被两两配成 **3 对**,
距离全是 0。原因在 dHash 的定义本身 —— 它只比较**相邻像素谁更亮**, 不看绝对
亮度, 所以纯红与纯蓝每一对相邻像素都是"不大于", 指纹完全相同。放到真实场景里,
一个装满纯色截图 / 占位图 / Logo 的文件夹会**整片互指**。

修法是 `phash.is_flat_gray()`(灰度极差小于阈值就认为指纹不携带信息), 这类图
**根本不进比对池**, 并把数量报成 `flat`。改完再跑: 真重复(p1.png 与重压缩后的
p2.jpg)配成一对、距离 0;三张纯色被计入 `flat`、不再互指;噪点图不配对。

⚠️ **这条是冒烟抓到的, 不是单测。** 单测用的是 monkeypatch 掉的假指纹, 永远走
不到"真解码出一片纯色"那条路 —— 这正是第 ⑦ 条心法之外还要补一条的理由:
**替身越忠实, 越看不见替身覆盖不到的那一层。** 所以补的两条用例一条直接喂
灰度字节给 `is_flat_gray`(判据挂在数据上, 不挂在 ffmpeg 上), 另一条走
`duplicates` 的全链路验 `flat` 计数。

顺带: 替身这次也换了写法 —— 不去替 `dhash_from_gray`(那样 `phash.distance`
这段真逻辑就不跑了, 用例验到的成了替身自己), 而是**反推**一组灰度字节让真的
dHash 算出来等于想要的值(`_raw_for_dhash`)。

**没做的那些不是"不该做"**: 人脸/语义搜索/地图属于另一档投入(模型 + 索引 + 一套
新的 UI), 而颜色搜索/智能文件夹需要一套"属性的属性"语义 —— 现在记在这里,
是为了让下一个人知道它们是**被评估过然后推迟的**, 不是被遗忘的。

> ⚠️ **2026-09-25 更正**: 上一版把**评分**也列进了"需要'属性的属性'语义"那一档,
> 那是**记错了**。真正需要那套语义的是**智能文件夹**(规则的组合与嵌套), 而评分
> 在 V42 落地时证明只是 `resources` 上的一列整数 —— 见 §23.2。这条记录被改掉,
> 正是 C 类坑("断言的前提会失效")在文档上的那一面: 回头核一遍, 别让一句当初
> 的判断变成后人眼里的"评估过所以不做"。

---

## 23. 五个方向各自对标同类产品, 再吸收一轮（V42，2026-09-25 一）

上一轮(§22.8)对标的是"本地相册"这一个方向。这一轮把产品按**五个方向**拆开,
每个方向各找同类产品 —— 因为"下载器"和"素材管理器"的约束完全不同, 混在一起
照抄会抄到别人的约束上去。

| 方向 | 对标的 | 吸收了 | 明确推迟 |
| --- | --- | --- | --- |
| 站点/图集采集 | gallery-dl / yt-dlp | **URL 归档**、元数据 sidecar(已有) | 更多站点插件(先跑 `probe_site.py`) |
| 下载与调度 | aria2 / JDownloader / N_m3u8DL-RE | 分段并发与校验(已有)、解密与字幕合并 | 多镜像并发抢带宽、定时队列 |
| 素材整理 | Eagle / Billfish / TagStudio / digiKam | **评分**、**体检视图**(`special:` 类筛选) | 颜色搜索、智能文件夹、人脸 |
| 重复与清理 | Czkawka / dupeGuru | **重复分组 + 建议保留**、**扩展名与文件头不符** | 相似(非重复)分级、按内容分块哈希 |
| 浏览与回忆 | Immich / Google Photos | 往年今日(已做)、收藏(已有) | 人脸聚类、CLIP 语义搜索、地图(合规 + 重依赖) |

### 23.1 先说清楚"谁跟我们是同一件事"

这是两轮对标里最值钱的一条经验: **Immich 的 External Library 与我们本地相册集是
同一个契约**(`:ro` 挂进来、只建索引), 所以它的每个设计都能直接搬; 而 Google Photos
是"你把所有照片交给它"的托管模型, 它的"回忆"建立在全量人脸与地理之上 —— 照抄那个
形态, 等于先要求用户把目录交出来, 而那正是我们**只读**承诺要避免的。

同理, Czkawka 的 "bad extensions" 之所以能直接搬, 是因为它和我们一样面对
**不可信来源的文件**(源站返回什么就存什么); 而 dupeGuru 面向的是"我自己攒出来的
重复", 它的默认动作是删 —— 那个默认动作在我们这儿不能用(dHash 会误判)。

### 23.2 评分: 为什么收藏之外还要有它

`resources` 加一列 `rating INTEGER NOT NULL DEFAULT 0`。与 `favorite` **并存而不是
合并**: 收藏回答"还要不要再看到它"(布尔, 只能分成两堆), 评分回答"它有多好"
(序数, **能排序**)。在几千张里挑"最好的那几张"时, 布尔做不到排序 —— Eagle /
digiKam / TagStudio / Immich 全都两套并存, 理由就在这里。

两条纪律:

* **上限由后端硬拦**(越界 `ValueError` → 400), 不是交给界面。评分会被
  `min_rating` 筛选消费, 一个 7 星的资源会让"≥5 星"这个口径说不清; 而界面那一侧
  只负责画星星, 它并不知道上限是多少。
* **`0` 表示"还没评过", 不是"0 分"**。所以"未评分"这个筛选读的是 `rating = 0`
  这一列, 界面不许靠"看起来没有星星"去猜 —— 那是把状态画成了外观, 以后换皮肤
  就得重新解释一遍。

### 23.3 体检视图: "没有这类"与"没算出这类"必须是两个结论

`SPECIAL_FILTERS` 把九条跨字段判断各定义一次(未打标签 / 未评分 / 已评分 /
疑似重复 / 内容损坏 / 文件已丢失 / 名字与内容不符 / 没量到尺寸 / 没量到时长),
`/library/facets` 给每一条的**条数**。

为什么必须单独成项: 这类问题**不是某个字段的取值**, 而是一条判断。指望用户自己
拼出"没有标签 AND 类型是图片"是不现实的 —— TagStudio 为此发明了 `special:` 查询
语法, Czkawka 干脆做成十种扫描模式, 说的是同一件事。

⚠️ 判据是 **0 就是 0**: 空库上每个维度都要有条目且 `n == 0`, 而不是返回空列表
(第 26 条)。界面不许把 0 显示成 `—` 或"未知"。评分区间同理 —— **六档全给**
(0..5), 否则"5 星有多少"这个问题没法问。

另外, 未知维度键**抛 `ValueError` → 400**, 不是静默忽略。静默忽略的表现是
"点了没反应", 那比报错难查得多(第 9 条: 判据挂在代号上, 而"未知代号"本身
必须是个错误)。

### 23.4 重复分组: 只说"它们重复"是把最难的一步丢回给人

`duplicate_of` 原本是一条**平铺的指向**(每条只写"我像谁")。`duplicate_groups()`
把它还原成组, 并给出**建议保留的那一条 + 理由代号**:

1. 像素最高的(同图的不同尺寸版本里, 大图几乎总是要留的那个);
2. 像素数相同(或都没量到)则体积大的;
3. 都一样则 **id 最小**那条 —— 它先到, 最像原件。

`keep_reason` 是**代号**(`highest_res` / `largest` / `oldest`), 中文由 `reasons`
下发。三条理由各有一条用例走它自己的那条路 —— 混在一个断言里的话, "理由对不对"
其实没人验。

⚠️ **只标记不删**的一贯原则在这里继续适用(dHash 会误判, 实测纯色图互相距离 0,
见 §22 末尾)。界面上"只保留建议项"默认**只删记录不动文件**, 且二次确认里写明了。

### 23.5 URL 归档: 增量在换机器之后为什么还能继续

对标 yt-dlp / gallery-dl 的 `--download-archive`。它解决的是一个具体场景:
**换机器 / 重装之后增量断掉**。资源库当然知道自己下过什么, 但那份记录只活在这一个
库文件里; 归档是**能随身带走的纯文本**(导出就是 `.txt`), 带到新机器上就能让增量
继续跳过旧内容 —— 而增量是订阅巡检唯一不重复下载的依据。

三条纪律:

* **`added` 与 `skipped` 两个数都给**(第 27 条)。只回"导入成功"的话, 用户没法
  判断这份归档是不是真被吃进去了 —— 而"导入了但没生效"正是这类功能最典型的失败
  方式。
* 它**不是**资源库的下游副本: 归档里的 URL 在本机可能根本没有对应文件(那是别人 /
  别的机器下的)。所以它只用于**提示与可选的跳过**, 永远不参与"这个文件在不在、
  能不能删"的判断 —— 那归 `resources.local_path` 管。
* 解析同时吃 yt-dlp 的 `extractor id` 两列格式: 那不是 URL, 但它同样能回答"这个
  我下过没有", 而且用户很可能直接贴过来。行内的 sha(32/40/64 位十六进制)顺手
  记下, 判重只按 URL。

### 23.6 扩展名与文件头不符: 一个"全程不报错"的场景

`core/filekind.py`(对标 Czkawka 的 "bad extensions")。它答的是与 `mediacheck`
**不同的问题**: 那个答"文件完不完整"(容器长度算术), 这个答"它是不是它自称的那个
东西"(magic sniff)。

最典型的现场: CDN 回了一个 **HTML 错误页**, 而 `Content-Length` 正是那个 HTML 的
长度 —— 于是下载层的长度校验**通过**, 文件被当成图片落盘, 名字是 `00001.jpg`。
之后缩略图、dHash、manifest 全都建立在一份 HTML 上, **全程不报错**; 只有用户双击
打开时才看到"文件已损坏"。

所以它单列一类 `KIND_MISMATCH`, 与 `corrupt` **分开计数**: 前两类(缺失/截断)重下
有救, 这一类重下还是它。合成一个数字就没法指导下一步动作。

判据的松紧与 `mediacheck` 同款(**宁可漏报, 不可误报**): 不认识的扩展名、认不出的
文件头一律不下结论; `ftyp` 家族(mp4/mov/m4a/heic/avif)视为同一族 —— 靠 brand 细分
会把 `.heic` 判成"其实是 mp4", 那是误报: 它本来就是那个文件, 只是我们没细看。

### 23.7 这一轮的实证: 门禁真的会红

改完跑全量, `test_gateguard.py::test_the_readme_case_count_matches_what_pytest_really_collects`
**当场红了**, 报错是"本机实测收集 **1341** 条, 而 README 只写了 [1309, 1310]"。

这条值得记下来, 因为它证明 R-CI-1 不是摆设: 文档上的数字是**唯一当前口径**, 而
"记着待会儿改"这种自觉在四个版本里失败过至少两次。改数字的同时把特性清单补上
(`评分·体检视图·重复分组·URL归档·文件头sniff`), 因为那一行还是"下一版要做什么"
的索引。

### 23.8 仍然推迟的(不是"不该做")

* **语义搜索 / 人脸聚类 / 地图**: 另一档投入(模型 + 索引 + 一套新 UI), 且地图涉及
  合规。
* **颜色搜索**: 要额外的颜色索引(逐图算主色), 而它与"按内容找"是同一档投入。
* **智能文件夹(嵌套规则版)**: ⚠️ 这条的**理由在 V43 被部分修正了** —— 见 §24.4。
  平铺版(存一组筛选条件)并不需要"属性的属性"语义, V43 已落地; 需要那套语义的是
  **规则嵌套/组合**(`(标签=系列 AND 星级≥4) OR 未评分`)。
* **多镜像并发抢带宽 / 定时队列**: 下载方向上的"锦上添花", 优先级低于更多站点插件。
  ⚠️ **周期巡检其实早就在**(`watches` 表 + `due_watches` + 增量模式 + `interval_minutes`),
  拿"定时队列"当缺口之前先去 `task_manager._scheduler_loop` 看一眼。
* **HLS 直播**: 缺真实站点样本, 没有靶子就不该写。

## 24. 资源库的排序与保存的搜索（V43，2026-09-25 二）

### 24.1 这一轮补的是**入口**, 不是能力

资源库此前能筛(关键词/相册/标签/星级/体检维度)但**不能排**, 也没有"把我常用的这组
条件存下来"的地方。Eagle / digiKam / PhotoPrism / Immich 都有这两样, 而它们是**同一
类东西**: 都回答"我怎么再回到这一屏", 而不是"库现在什么样"(那是 §22.8 / §23 的四项)。
采集侧早就有"预设"(`presets.js`), 但那是**采集参数**的预设, 与资源库的筛选条件
是两码事 —— 名字像不等于同一件事。

### 24.2 排序: 翻译点只能有一个

档位定义在 `core/database.py` 的 **`LIBRARY_SORTS`** 一张表里(键 → (列表达式, 中文名)),
`library_order_by()` 是唯一的翻译点, 中文名由 `/library` 与 `/library/facets` 同源下发。

* **拿前端传来的字符串直接拼 `ORDER BY` 是能跑通的注入** —— 它不报错、结果也对,
  所以没人会发现。所以未知代号**抛错 → 400**, 而不是回退到默认档位。
  静默回退的后果不是"排序没生效"这么轻: 用户会认为"这软件的大小排序是坏的",
  而日志里一个字都没有。
* 除主键档位外**每一档都拼一个次级键 `r.id`**。同值行(同一秒落盘 / 同样大小 /
  同为 5 星)在 SQLite 里的相对次序由扫描顺序决定, 翻页时同一行可能**出现在两页**、
  也可能**一页都不出现**。这类错不报错, 只表现为"我明明看到过那一条";
  用例用 `limit=1` 逐页取, 再和整表逐位比。
* `/library` 回显**实际生效**的 `sort`/`order`。不回显的话, 一次被兜底的请求会让界面
  高亮一个错的档位 —— 而"高亮"和"真实顺序"不一致时, 人是不会去怀疑界面的。

### 24.3 保存的搜索: 条件是**具名参数**, 不是 SQL

`library_searches` 表(id / name / params(JSON) / created_at)。时刻列按新表约定是
**`created_at` + REAL(epoch 秒)**。

* 存 JSON **不等于**存了一段可执行的 SQL: 回灌时仍然是 `library_count(**params)` /
  `library_filters(**params)` 那些具名参数, 白名单(`LIBRARY_QUERY_KEYS`)之外一键
  都进不来。这是"存用户意图"与"拼 SQL"的分界。
* **坏条件必须在保存时拒绝**。一个存进去的非法 `special` 会让*每次列出搜索*都抛
  异常 —— 整个侧栏打不开。保存时拒绝是唯一能保证"存下来的都跑得动"的位置。
* 同名**覆盖**而不是报"已存在": 用户的常态是"改完条件再存", 报错只会逼他先删再建。
  但要如实回 `created=False`, 让他知道刚才那条被换掉了。

### 24.4 ⚠️ 又一条"推迟理由"过期了

§23.8 把"智能文件夹"归为需要**"属性的属性"语义**(规则的组合与嵌套)、因此推迟。
V43 落地的是**平铺版** —— 存一组筛选条件 —— 而它根本不需要那套语义: 它只是把
`query` 里的九个字段序列化下来。真正需要"属性的属性"的是**规则嵌套/组合**
(`(标签=系列 AND 星级≥4) OR 未评分`), 那仍然推迟。

这与 §22.8 末尾 V42 更正"评分被记错"是**同一个错法**: 把"我们还没做的一件更大的
事"的理由, 顺手安在了"其实很小的一步"上。两次都被"回代码里看一眼要多少活"戳破。
规则: **推迟理由要写在"最小可做版本"旁边**, 否则它保护的是想象力而不是判断。

### 24.5 "0 就是 0" 与 "算不出来"

侧栏每个保存的搜索都带 `count`。两条判据在这里同时生效, 而且**必须是两个结果**:

| 情形 | `count` | `broken` | 界面 |
| --- | --- | --- | --- |
| 数过了, 这个搜索现在没有东西 | `0` | `false` | `0`(不是 `—`) |
| 条件在库里读不出来(手改过库/旧值) | `null` | `true` | `—` + 虚线边框 |

**读不出来时绝不能去数**: 空条件数出来的是"整个库的条数", 于是这条坏搜索会被伪装成
"一条正常但很宽的搜索", 用户看到四位数只会以为自己的条件存丢了。坏的那条也**不隐藏**
—— 让它消失等于把"坏了"变成"没有"。

### 24.6 应用一个搜索 = **整体替换**条件

侧栏上的 `count` 是按**存下来的那组条件**算的。若应用时是逐项合并(带着上一轮的标签/
星级继续查), 列表条数就会与那个数字对不上 —— 而这**不报错**, 只表现为"这个搜索的数
好像不太准", 用户多半会去怀疑自己的记忆。

同理: 手动改任何筛选条件都要**取消当前搜索的高亮**, 否则侧栏一直亮着"搜索 A"而列表
早就是别的东西了。

### 24.7 这一轮的实证

改完跑全量, `test_gateguard.py::test_the_readme_case_count_matches_what_pytest_really_collects`
**又红了**, 报"本机实测收集 **1367** 条, 而 README 只写了 [1341, 1342]"。
