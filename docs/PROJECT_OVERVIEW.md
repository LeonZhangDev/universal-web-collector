...
真实站点验证可能发出网络请求并创建产物，运行前应检查脚本的输入和输出位置。本机最近一次全量验证结果为 557 项测试通过，另有 `verify_output.py` 40 项、`verify_hls.py` 18 项断言通过，`selfcheck.py` 站点声明自洽。测试数量随功能增长，最新结果以当次运行输出为准。
...
| 下载速度与阶段进度 | 部分实现：进度条与详情内速率估算（张/分），字节级速率待补 |
| 代理支持 | 已实现：任务级代理、多代理轮换、注入下载会话 |
| 任务通知 | 已实现：通知中心 + 可选 SMTP 推送 |
| 统计可视化 | 已实现：SVG 折线 + 饼图 + 失败原因 + 去重报表 |

## 后续可做

截至 V31（`609f2c7`），此前的建议清单已全部处理完毕，无遗留项。下表为**尚未实施**的部分，
均为 P2 及以下，需要时再动：

| 优先级 | 建议 | 价值与范围 |
| --- | --- | --- |
| P2 | 跨任务资源库 | 按相册、来源、标签浏览内容；先明确共享文件引用与删除规则 |
| P2 | 字节级实时速率曲线 | 后端只回传百分比，需先补字节增量字段（`resources.bytes_done`）才能画真实 sparkline |
| P2 | 代理健康检查与自动降级 | 轮换目前按序号取模，可加"失败换线 + 熔断"（`ProxyPool` 内加失败计数即可） |
| 按需求 | 新站点插件 | 针对实际常用站点增加适配与样本测试，复用现有下载和清单机制 |

> 「下载速度与阶段进度」一行标注为"部分实现"：当前按**张/分**估算，字节级速率依赖上表的
> 字节增量字段，属同一件事，一并留到那时做。
